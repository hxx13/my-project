...
var mpMedia = require('../../../utils/mpDisplayMedia.js');
...
var CAGE_SHELF_PAGE = '/package-student/pages/studentCageShelf/index';
...
function normalizeStatusHistory(list) {
  ...
  list.forEach(function(h) {
    try { h._imgs = mpMedia.mapMediaUrlList(JSON.parse(h.imagesJson || '[]')); } catch (e) { h._imgs = []; }
...
/** 状态照片对象里有没有可展示内容。有 key ≠ 有内容：标了状态没传图时是空数组，整节会渲染成空盒子。 */
function hasStatusContent(sp) {
  if (!sp) return false;
  if (typeof sp._note === 'string' && sp._note) return true;
  return Object.keys(sp).some(function (k) {
    return k !== '_note' && Array.isArray(sp[k]) && sp[k].length > 0;
  });
}

/**
 * 状态照片是按状态分桶存的（key = 状态表单字段名，值是 URL 数组，`_note` 是字符串）。
 * 库里存的是 `/api/upload/files/...` **相对路径**，`<image>` 不认（渲染成空框）——
 * 逐桶转绝对地址，与台账照片、物资/报修走同一条转换链（utils/mpDisplayMedia.js）。
 */
function absMediaMap(sp) {
  var out = {};
  if (!sp || typeof sp !== 'object' || Array.isArray(sp)) return out;
  Object.keys(sp).forEach(function (k) {
    out[k] = Array.isArray(sp[k]) ? mpMedia.mapMediaUrlList(sp[k]) : sp[k];
  });
  return out;
}

/**
 * 能否在详情弹窗里直接删/管状态照片 —— 与后端 `/local/annotate` 的状态照片闸**同口径**：
 * `isStudent(u) || !canUseMode(u,'edit')` 才拒。所以这里 = 非学生 **且** 矩阵给了「状态」模式。
 * visibleModes 为空表示后端没下发，按本地回退（教职工那份 base 里含 'edit'）。
 */
function canEditStatusPhotos(isStaffView, visibleModes) {
  if (!isStaffView) return false;
  if (!visibleModes || !visibleModes.length) return true;
  return visibleModes.indexOf('edit') >= 0;
}

/** 照片桶 key → 给人看的名字（明细 `SF_` 前缀见后端 CageStatusIntervalService.DETAIL_STATUS_PREFIX） */
function orphanPhotoLabel(key) {
  if (key.indexOf('SF_') === 0) return '特殊饲养明细';
  return '状态照片';
}

/**
 * 没有对应状态 chip 的照片桶（明细 `SF_*` 与历史遗留 `_status`）。
 * **必须渲染出来**：不渲染 = 这些照片既看不见、也就永远删不掉。
 */
function orphanPhotoGroups(sp, chips) {
  var known = {};
  (chips || []).forEach(function (c) { if (c && c.statusField) known[c.statusField] = true; });
  var out = [];
  Object.keys(sp || {}).forEach(function (k) {
    if (k === '_note' || known[k]) return;
    if (!Array.isArray(sp[k]) || !sp[k].length) return;
    out.push({ key: k, label: orphanPhotoLabel(k), urls: sp[k] });
  });
  return out;
}

/** 台账记录的 images_json：null / 非法串 / 非数组一律折成字符串数组 */
function parseExpImages(raw) {  if (!raw) return [];
  try {
    var a = typeof raw === 'string' ? JSON.parse(raw) : raw;
    return Array.isArray(a) ? a.filter(function(u) { return typeof u === 'string'; }) : [];
  } catch (e) {
    return [];
  }
}

/** 台账记录预处理出渲染字段：_time（提交时间，缺则创建时间）、_imgs、_brief（列表摘要，全文看「查看」） */
function decorateExpRecord(r) {
  if (!r) return r;
  // 台账存的是 /api/upload/files/... 相对路径；<image> 不认相对路径，必须转成绝对地址，
  // 与物资/报修同一条转换链（mpDisplayMedia → springAuth.toAbsoluteMediaUrl）。
  r._imgs = mpMedia.mapMediaUrlList(parseExpImages(r.imagesJson));
  r._time = String(r.submittedAt || r.createdAt || '').replace('T', ' ').substring(0, 16);
  var c = r.content || '';
  r._brief = c.length > 60 ? c.substring(0, 60) + '…' : c;
  return r;
}

/** 原始坐标 → 网格显示口径的位号（列转字母、行号取反），与 Web 的 displayPosition 同规则 */
function expPositionLabel(x, y) {
  if (x === null || x === undefined || y === null || y === undefined || x === '' || y === '') return '';
  var col = String.fromCharCode(64 + Number(x));
  return col + '-' + (11 - Number(y));
}
...
/**
 * 我提交的全部分笼/转移（不分类型）。
 * 与 Web 的「我的申请」同一个接口：分笼、转移两个 tab 都是从这一份里按 opType 过滤出来的。
 */
/**
 * 需要我审的分笼/转移 = 待我签的（/pending）+ 我已经审过的（/reviewed）。
 * 与 Web 的「审核申请」tab 同两个接口、同口径。
 */
...
Page({
  data: {
    ...
    expRecords: [],        // 已提交记录（每项预处理出 _time / _imgs）
    expDraft: null,        // 本人当前草稿（同人同笼位最多一条）
    expDraftContent: '',
    expDraftImages: [],
    expCouldView: true,    // false = 服务端没下发内容（非实验员本人），只渲染 *** 占位
    expCouldWrite: false,
    expOpen: false,        // 编辑器展开
    expHistOpen: true,    // 历史记录折叠态（默认折叠，点标题展开）
    expViewOpen: false,    // 单条记录「查看」弹窗
    expViewRec: null,      // 弹窗里那条记录
    expUploading: false,
    expSaving: false,
    detailStatusPhotos: {},
    detailHasStatusBlock: false,
    detailAnnotationLoading: false,
    /** 能否在详情弹窗里直接删状态照片（非学生 + 矩阵给了「状态」模式，与后端同一判据） */
    canEditStatusPhotos: false,
    detailStatusSaving: false,
    /** 没有对应状态 chip 的照片桶（`SF_*` 明细 / `_status` 遗留），照样要能看能删 */
    detailOrphanPhotos: [],
   ...
   */
  /** 我的申请已改为独立整页（4 个分类 tab），这里只负责跳过去 */
  onOpenMyClaims: function() {
    wx.navigateTo({ url: '/package-student/pages/myCageRequests/index' });
  },

  /* ══════════ 我的实验记录（按房间分组，含已失去权限/归档的历史笼位） ══════════ */

  onOpenMyRecords: function() {
    var self = this;
    self.setData({ myRecordsOpen: true, myRecordsLoading: true, myRecordsRooms: [] });
    springAuth.springRequest({
      url: '/api/local/experiment-record/mine',
      method: 'GET',
      data: {}
    }).then(function(res) {
      var up = unwrap(res);
      var rooms = (up.ok && up.data) || [];
      rooms.forEach(function(room, ri) {
        room._key = room.roomId || ('room-' + ri);
        room._open = true;
        room._sub = [room.campusName, room.floorName].filter(nonEmptyText).join(' / ');
        var total = 0;
        (room.cages || []).forEach(function(cage, ci) {
          cage._key = room._key + '#' + (cage.animalCageId != null ? cage.animalCageId : ci);
          cage._open = true;
          cage._pos = expPositionLabel(cage.positionX, cage.positionY) || '—';
          (cage.records || []).forEach(decorateExpRecord);
          total += (cage.records || []).length;
        });
        room._total = total;
      });
      self.setData({ myRecordsLoading: false, myRecordsRooms: rooms });
    }).catch(function() {
      self.setData({ myRecordsLoading: false });
      wx.showToast({ title: '加载我的实验记录失败', icon: 'none' });
    });
  },

  onCloseMyRecords: function() {
    this.setData({ myRecordsOpen: false });
  },

  onToggleMyRecordsRoom: function(e) {
    var key = e.currentTarget.dataset.key;
    var rooms = (this.data.myRecordsRooms || []).slice();
    for (var i = 0; i < rooms.length; i++) {
      if (rooms[i]._key === key) { rooms[i]._open = !rooms[i]._open; break; }
    }
    this.setData({ myRecordsRooms: rooms });
  },

  /** 笼位行折叠（一个房间里笼位可能很多，全展开会把别的房间顶没） */
  onToggleMyRecordsCage: function(e) {
    var key = e.currentTarget.dataset.key;
    var rooms = (this.data.myRecordsRooms || []).slice();
    for (var i = 0; i < rooms.length; i++) {
      var cages = rooms[i].cages || [];
      for (var j = 0; j < cages.length; j++) {
        if (cages[j]._key === key) { cages[j]._open = !cages[j]._open; this.setData({ myRecordsRooms: rooms }); return; }
      }
    }
  },

  /** 「我的实验记录」里点「查看」：与详情弹窗共用同一个记录详情弹窗 */
  onMyRecordsView: function(e) {
    var id = e.currentTarget.dataset.id;
    var found = null;
    (this.data.myRecordsRooms || []).forEach(function(room) {
      (room.cages || []).forEach(function(cage) {
        (cage.records || []).forEach(function(rec) { if (String(rec.id) === String(id)) found = rec; });
      });
    });
    if (!found) return;
    this.setData({ expViewOpen: true, expViewRec: found });
  },

  /** 「我的实验记录」里的照片预览：跨整个房间树汇图，别串到详情弹窗那份台账上 */
  onMyRecordsPreview: function(e) {
    var url = e.currentTarget.dataset.url;
    if (!url) return;
    var urls = [];
    (this.data.myRecordsRooms || []).forEach(function(room) {
      (room.cages || []).forEach(function(cage) {
        (cage.records || []).forEach(function(rec) {
          (rec._imgs || []).forEach(function(u) { if (u && urls.indexOf(u) < 0) urls.push(u); });
        });
      });
    });
  ...
  onCellTap: function(e) {
      ...
      if (animalCageId) {
        springAuth.springRequest({ url: '/api/local/annotate/' + animalCageId, method: 'GET', data: {} }).then(function(res) {
          ...
          if (up.ok && up.data) {
            ...
            self.setData({ editActionNote: note, editActionPhotos: mpMedia.mapMediaUrlList(all) });
          }
        });
      }
      return;
    ...
    self.setData({
      ...
      showCellDetail: true,
      // 实验记录台账：换笼位先清空，接口回来再填（原「一格一份文本」的 experimentDesc 已废弃）
      expRecords: [],
      expDraft: null,
      expDraftContent: '',
      expDraftImages: [],
      ...
      canEditStatusPhotos: canEditStatusPhotos(self.data.isStaffView, self.data.visibleModes),
    ...
    if (detailMeta.permitted) {
      ...
      self.loadExperimentRecords(cell);
...
loadCellAnnotation: function(cell) {
    ...
    springAuth.springRequest({
      ...
      // 转绝对地址：库里是相对路径，<image> 直接渲染会是空框（台账那处踩过同一个坑）
      statusPhotos = absMediaMap(statusPhotos);
      self.setData({
        detailStatusPhotos: statusPhotos,
        detailHasStatusBlock: hasStatusContent(statusPhotos),
        detailAnnotationLoading: false
      });
      self._refreshOrphanPhotos();
      self._refreshSectionNos();
  ...
  /* ══════════ 实验记录台账（追加式：一条一个时间戳，提交后只读、不可编辑不可删除） ══════════ */

  /** 拉台账：脱敏在服务端做完，canView=false 时 records/draft 都不下发 */
  loadExperimentRecords: function(cell) {
    var self = this;
    var animalCageId = cell && (cell.id || cell.animalCageId);
    if (!animalCageId) {
      self.setData({ expRecords: [], expDraft: null, expCouldView: true, expCouldWrite: false, expOpen: false, expHistOpen: true, expViewOpen: false, expViewRec: null });
      return;
    }
    springAuth.springRequest({
      url: '/api/local/experiment-record/' + animalCageId,
      method: 'GET',
      data: {}
    }).then(function(res) {
      self._applyExpState(unwrap(res));
    }).catch(function() {
      self.setData({ expRecords: [], expDraft: null });
    });
  },

  /**
   * 接口结果 → 渲染态。草稿只在「换了一条草稿」时回填编辑器，否则会把用户正在敲的字冲掉。
   */
  _applyExpState: function(up) {
    var self = this;
    if (!up || !up.ok || !up.data) {
      self.setData({ expRecords: [], expDraft: null });
      return;
    ...
    var d = up.data;
    var canView = d.canView !== false;
    var draft = canView ? (d.draft || null) : null;
    var prevId = self.data.expDraft && self.data.expDraft.id;
    var next = {
      expRecords: canView ? (d.records || []).map(decorateExpRecord) : [],
      expDraft: draft,
      expCouldView: canView,
      expCouldWrite: !!d.canWrite
    };
    if (draft && draft.id !== prevId) {
      next.expDraftContent = draft.content || '';
      next.expDraftImages = parseExpImages(draft.imagesJson);
      next.expOpen = true;
    } else if (!draft) {
      next.expOpen = false;
      if (prevId) { next.expDraftContent = ''; next.expDraftImages = []; }
    }
    self.setData(next);
  },

  /** 新增：有草稿时 wxml 就不出这个入口，这里再兜一道 */
  onExpRecordNew: function() {
    if (this.data.expDraft) return;
    this.setData({ expOpen: true, expDraftContent: '', expDraftImages: [] });
  },

  /** 历史记录折叠/展开（默认折叠，记录多时不把新增入口顶到屏幕外） */
  onExpToggleHistory: function() {
    this.setData({ expHistOpen: !this.data.expHistOpen });
  },

  /** 单条记录「查看」：列表里只给摘要，全文与全部照片放弹窗 */
  onExpViewRecord: function(e) {
    var id = e.currentTarget.dataset.id;
    var rec = (this.data.expRecords || []).filter(function(r) { return String(r.id) === String(id); })[0];
    if (!rec) return;
    this.setData({ expViewOpen: true, expViewRec: rec });
  },

  onExpCloseView: function() {
    this.setData({ expViewOpen: false, expViewRec: null });
  },

  onExpRecordInput: function(e) {
    this.setData({ expDraftContent: e.detail.value || '' });
  },

  onExpRecordRemoveImage: function(e) {
    var index = e.currentTarget.dataset.index;
    var images = (this.data.expDraftImages || []).slice();
    if (index >= 0 && index < images.length) {
      images.splice(index, 1);
      this.setData({ expDraftImages: images });
    }
  },

  /** 草稿照片上传（与状态照片同一套 uploadFileDirect） */
  onExpRecordChooseImage: function() {
    var self = this;
    if (self.data.expUploading) return;
    wx.chooseImage({
      count: 9,
      sizeType: ['compressed'],
      ...
      success: function(res) {
        ...
        self.setData({ expUploading: true });
        var uploaded = [];
        var next = function(idx) {
          if (idx >= res.tempFilePaths.length) {
            self.setData({ expUploading: false });
            if (uploaded.length > 0) {
              self.setData({ expDraftImages: (self.data.expDraftImages || []).concat(uploaded) });
            }
            return;
          }
          springAuth.uploadFileDirect(res.tempFilePaths[idx], {}).then(function(url) {
            ...
            wx.showToast({ title: '有图片上传失败', icon: 'none' });
            next(idx + 1);
          });
        };
        next(0);
  ...
  /** 台账照片预览：只汇台账自己的图，和状态照片各自预览互不干扰 */
  onExpPreviewImage: function(e) {
    var url = e.currentTarget.dataset.url;
    if (!url) return;
    var urls = [];
    var push = function(u) { if (u && urls.indexOf(u) < 0) urls.push(u); };
    (this.data.expRecords || []).forEach(function(r) { (r._imgs || []).forEach(push); });
    (this.data.expDraftImages || []).forEach(push);
    push(url); // 兜底：被点的那张一定在列表里
    wx.previewImage({ current: url, urls: urls });
  },

  onExpRecordSaveDraft: function() { this._submitExpRecord('draft'); },
  onExpRecordSubmit: function() { this._submitExpRecord('submit'); },

  /** @param action 'draft' 存草稿（有则覆盖） | 'submit' 提交成一条新记录 */
  _submitExpRecord: function(action) {
    var self = this;
    var cell = self.data.selectedCell;
    if (!cell || self.data.expSaving || !self.data.expCouldWrite) return;
    var animalCageId = cell.id || cell.animalCageId || '';
    if (!animalCageId) return;
    var content = self.data.expDraftContent || '';
    var images = self.data.expDraftImages || [];
    if (action === 'submit' && !content.trim() && images.length === 0) {
      wx.showToast({ title: '请先填写文字或添加照片', icon: 'none' });
      return;
    }
    self.setData({ expSaving: true });
    springAuth.springRequest({
      url: '/api/local/experiment-record',
      method: 'POST',
      data: { animalCageId: animalCageId, action: action, content: content, imagesJson: JSON.stringify(images) }
    }).then(function(res) {
      var up = unwrap(res);
      self.setData({ expSaving: false });
      if (!up.ok) {
        wx.showToast({ title: up.message || '保存失败', icon: 'none' });
        return;
      }
      self._applyExpState(up);
      wx.showToast({ title: action === 'submit' ? '已新增一条实验记录' : '草稿已保存', icon: 'none' });
    }).catch(function(e) {
      self.setData({ expSaving: false });
      wx.showToast({ title: (e && e.message) || '保存失败', icon: 'none' });
    });
  ...
  /**
   * 照片预览（状态照片一线）：把当前弹窗里状态照片汇成一个可左右滑动的大图列表。
   * 实验记录自己的照片走 onExpPreviewImage —— 台账独立成卷，别和状态照片混在一个列表里。
   */
  ...
  /**
   * 重算「没有对应状态 chip 的照片桶」。照片桶来自标注接口、chips 来自表单接口，
   * 两条异步路径谁后到都要重算一次，否则桶会错位（与 _refreshSectionNos 同理）。
   */
  _refreshOrphanPhotos: function() {
    var sp = this.data.detailStatusPhotos || {};
    var chips = (this.data.cellDetail && this.data.cellDetail.statusChips) || [];
    this.setData({ detailOrphanPhotos: orphanPhotoGroups(sp, chips) });
  },

  /**
   * 删一张状态照片。两个关键点：
   * ① **立即落库**（走 `/local/annotate` 的 statusPhotos 分支，与「状态模式」上传同一道闸）——
   *    详情弹窗已经不挂保存按钮了，只改内存等于白删；
   * ② 桶空了就**删键**而不是留空数组，否则那个状态的照片区会一直留一个空盒子；
   * ③ 服务端失败要回滚本地，不能让人以为删掉了。
   */
  onDetailRemoveStatusPhoto: function(e) {
    var self = this;
    var field = e.currentTarget.dataset.field;
    var index = Number(e.currentTarget.dataset.index);
    if (!self.data.canEditStatusPhotos || self.data.detailStatusSaving) return;
    var cell = self.data.selectedCell;
    var animalCageId = cell && (cell.id || cell.animalCageId);
    var sp = self.data.detailStatusPhotos || {};
    if (!animalCageId || !Array.isArray(sp[field]) || !(index >= 0 && index < sp[field].length)) return;

    var nsp = {};
    for (var k in sp) { if (Object.prototype.hasOwnProperty.call(sp, k)) nsp[k] = sp[k]; }
    nsp[field] = sp[field].slice();
    nsp[field].splice(index, 1);
    if (!nsp[field].length) delete nsp[field];

    var prev = sp;
    self.setData({ detailStatusPhotos: nsp, detailStatusSaving: true, detailHasStatusBlock: hasStatusContent(nsp) });
    springAuth.springRequest({
      url: '/api/local/annotate',
      method: 'POST',
      data: { animalCageId: animalCageId, statusPhotos: JSON.stringify(nsp) }
    }).then(function(res) {
      var up = unwrap(res);
      self.setData({ detailStatusSaving: false });
      if (!up.ok) {
        self.setData({ detailStatusPhotos: prev, detailHasStatusBlock: hasStatusContent(prev) });
        wx.showToast({ title: up.message || '删除失败', icon: 'none' });
        return;
      }
      wx.showToast({ title: '已删除', icon: 'none' });
      self._refreshOrphanPhotos();
    }).catch(function(err) {
      self.setData({ detailStatusPhotos: prev, detailHasStatusBlock: hasStatusContent(prev), detailStatusSaving: false });
      wx.showToast({ title: (err && err.message) || '删除失败', icon: 'none' });
    });
  },

  /** 没有对应状态 chip 的照片桶（明细 `SF_*` 与历史遗留 `_status`）—— 同样要能看能删 */
  onDetailRemoveOrphanPhoto: function(e) {
    this.onDetailRemoveStatusPhoto(e);
  },
