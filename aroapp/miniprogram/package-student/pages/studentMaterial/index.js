...
Page({
  ...
  onLoad() {
    ...
    const ok =
      !!token &&
      pagePermission.canAccessMiniPage('/package-student/pages/studentMaterial/index', role, 'STUDENT');
    if (!ok) {
  ...
  async _afterSubmitSuccess() {
    ...
    wx.navigateTo({ url: '/package-student/pages/studentMaterialRequests/index' });
  },
  ...
  goMine() {
    const role = wx.getStorageSync(springAuth.KEYS.ROLE) || '';
    if (!pagePermission.canAccessMiniPage('/package-student/pages/studentMaterialRequests/index', role, 'STUDENT')) {
      wx.showToast({ title: '无权限', icon: 'none' });
      return;
    }
    wx.navigateTo({ url: '/package-student/pages/studentMaterialRequests/index' });
  ...
  goManage() {
    const role = wx.getStorageSync(springAuth.KEYS.ROLE) || '';
    if (!pagePermission.canAccessMiniPage('/package-supplies/pages/materialAdmin/index', role, 'STAFF')) {
      wx.showToast({ title: '无权限', icon: 'none' });
      return;
    }
    wx.navigateTo({ url: '/package-supplies/pages/materialAdmin/index' });
  },
});
