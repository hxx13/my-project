...
function mapAroTraineeRow(row) {
  ...
  return {
    ...
    // 审核与评分是两个独立的 ▼，各自管自己的下拉项，别共用同一个标志
    _expandAudit: false,
    _expandScore: false,
  };
}
...
/** 转移/分笼的一行拍平成表格单元（只给表格视图用，卡片视图的逻辑不动） */
function buildCageOpTableRow(r) {
  var slots = r.signSlots || [];
  var pick = function (role) {
    for (var i = 0; i < slots.length; i += 1) {
      if (slots[i].role === role) return slots[i];
    }
    return null;
  };
  var signCell = function (s) {
    if (!s) return '—';
    return s.text + (s.who ? ' · ' + s.who : '');
  };
  // 源笼位在表格里只给「校区 / 房间 / 架子」：shelveName 自带位号，再缀一串「坐标 X-Y」是重复信息
  var srcParts = [];
  if (r.campusName) srcParts.push(r.campusName);
  if (r.roomName) srcParts.push(r.roomName);
  if (r.shelveName) srcParts.push(r.shelveName);
  // 目标同理，只是目标可能是多个（分笼 1:多）
  var targetLocs = Array.isArray(r.targets) ? r.targets : [];
  var targetParts = targetLocs
    .map(function (t) {
      var tp = [];
      if (t.campusName) tp.push(t.campusName);
      if (t.roomName) tp.push(t.roomName);
    ...
    .filter(Boolean);
  var targetCount = Array.isArray(r.targetAnimalCageIds) ? r.targetAnimalCageIds.length : 0;
  var targetText = targetParts.length
    ? targetParts.join('；')
    : (r.isDivide ? '共 ' + targetCount + ' 个笼位' : '—');
  return {
    id: r.id,
    isPending: String(r.status || '').toLowerCase() === 'pending',
    typeText: r.isDivide ? '分笼' : '转移',
    applicantText: r.applicantText || '—',
    sourceText: srcParts.join(' / ') || '—',
    targetText: targetText,
    originText: signCell(pick('ORIGIN')),
    destText: signCell(pick('DEST')),
    vetText: signCell(pick('VET')),
    statusText: r.statusText || '—',
    statusTagClass: r.statusTagClass || '',
    createdAtText: r.createdAtText || '—',
    reviewedAtText: r.reviewedAtText || '—',
    reviewerText: r.reviewerName || '—',
    reasonText: r.rejectReason || r.reason || '—',
    showPlainOps: !!r.showPlainOps,
...
Page({
  data: {
    ...
    cageTransferView: 'card',
    /** 表格的横向滚动位：切换视图时归零（scroll-view 会记住上次位置） */
    copScrollLeft: 0,
  },
  ...
  onShow() {
    ...
    if (!pagePermission.guardPageOnShow(this, '/package-student/pages/studentReviewHub/index', role, 'STAFF')) return;
  ...
  async loadDashboard(opts = {}) {
    ...
    try {
      ...
      this.setData({
        ...
        // 表格视图：待审 + 已审拍平成同一张表
        copTransferRows: cageTransferPending
          .concat(cageOpsReviewedByType.transfer)
          .map(buildCageOpTableRow),
        ...syncTabMeta(counts),
      });
  ...
  onToggleAroMore(e) {
    ...
    // data-kind 区分是「通过」那个 ▼ 还是「合格」那个 ▼，各展开各的
    const field = e.currentTarget.dataset.kind === 'score' ? '_expandScore' : '_expandAudit';
  ...
  /** 转移审核 卡片 / 表格 切换（对照订购记录的表格视图） */
  onCageTransferView(e) {
    const v = e.currentTarget.dataset.view === 'table' ? 'table' : 'card';
    // 0/1 交替：scroll-left 只在值变化时才生效，恒给 0 拉不回被记住的位置
    this.setData({ cageTransferView: v, copScrollLeft: this.data.copScrollLeft === 0 ? 1 : 0 });
  },
