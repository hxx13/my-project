...
/** yyyy-MM-dd HH:mm（24h、不含秒）——紧凑场景用；与 frontend/src/utils/beijingTime.ts 同名函数输出一致 */
function formatBeijingDateTimeMinute(v) {
  ...
  try {
    ...
    const parts = fmt.formatToParts(d);
    const g = (t) => (parts.find((x) => x.type === t) || {}).value || '';
    return `${g('year')}-${pad2(Number(g('month')))}-${pad2(Number(g('day')))} ${pad2(Number(g('hour')))}:${pad2(Number(g('minute')))}`;
  } catch (e) {
