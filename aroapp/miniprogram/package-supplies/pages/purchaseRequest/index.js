...
Page({
  ...
  async onShow() {
    ...
    if (!pagePermission.guardPageOnShow(this, '/package-supplies/pages/purchaseRequest/index', role, 'STAFF')) return;
    const cap = await fetchCapabilitySummaryMap({});
    const canProcess = !!(cap.PURCHASE || {}).canProcess;
  ...
  async loadData(options) {
    ...
    try {
      ...
      if (canProcess) {
        try {
          const keys = mapped.map((r) => ({ bizType: 'PURCHASE', bizId: r.id }));
          const flags = await notificationReadSync.fetchUnreadBizFlags(keys);
          mapped = mapped.map((r) => ({
            ...r,
            hasUnreadNotice: !!flags[notificationReadSync.toBizCompositeKey('PURCHASE', r.id)],
          }));
        } catch (e) {
          /* 未读标记失败不影响列表 */
        }
  ...
  async loadRecycle() {
    try {
      const res = await springAuth.springRequest({
        url: '/api/purchase/orders/recycle',
        method: 'GET',
  ...
  async markOrderRead(e) {
    ...
    try {
      const parsed = await notificationReadSync.markReadByBiz('PURCHASE', id);
      if (!parsed.ok) throw new Error(parsed.message);
      this.setData({
  ...
  startOrder(e) {
    ...
    wx.showModal({
      ...
      success: async (r) => {
        ...
        try {
          const res = await springAuth.springRequest({
            url: `/api/purchase/orders/${encodeURIComponent(id)}/start`,
            method: 'PATCH',
            data: {},
          });
          const parsed = parseResponse(res);
  ...
  async completeOrder() {
    ...
    try {
      const res = await springAuth.springRequest({
        url: `/api/purchase/orders/${encodeURIComponent(this.data.completingId)}/complete`,
        method: 'PATCH',
        data: {
  ...
  deleteOrder(e) {
    ...
    wx.showModal({
      ...
      success: async (r) => {
        ...
        try {
          const res = await springAuth.springRequest({
            url: `/api/purchase/orders/${encodeURIComponent(id)}`,
            method: 'DELETE',
            data: {},
          });
          const parsed = parseResponse(res);
  ...
  async onRestoreRecycle(e) {
    ...
    if (!id) return;
    const res = await springAuth.springRequest({
      url: `/api/purchase/orders/recycle/${encodeURIComponent(id)}/restore`,
      method: 'POST',
      data: {},
    });
  ...
  onPurgeRecycleOne(e) {
    ...
    wx.showModal({
      ...
      success: async (r) => {
        if (!r.confirm) return;
        const res = await springAuth.springRequest({
          url: `/api/purchase/orders/recycle/${encodeURIComponent(id)}`,
          method: 'DELETE',
          data: {},
        });
        const p = parseResponse(res);
  ...
  onPurgeRecycleSelected() {
    ...
    wx.showModal({
      ...
      success: async (r) => {
        if (!r.confirm) return;
        const res = await springAuth.springRequest({
          url: '/api/purchase/orders/recycle/purge',
          method: 'POST',
          data: { ids },
        });
        const p = parseResponse(res);
  ...
  withdraw(e) {
    ...
    wx.showModal({
      ...
      success: async (r) => {
        ...
        try {
          const res = await springAuth.springRequest({
            url: `/api/purchase/orders/${encodeURIComponent(id)}/withdraw`,
            method: 'POST',
            data: {},
          });
          const parsed = parseResponse(res);
