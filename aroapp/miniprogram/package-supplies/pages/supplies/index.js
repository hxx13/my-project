...
Page({
  ...
  onLoad(options) {
    ...
    const mallPath = '/package-supplies/pages/supplies/index';
    const adminMallOk =
  ...
  async onShow() {
    ...
    this.setData({
      ...
      showAdminEntry:
        superOk &&
        !!admin.canProcess &&
        pagePermission.canShowMiniEntry('mine', '/package-supplies/pages/suppliesAdmin/index', role, 'SUPER_ADMIN'),
  ...
  goMine() {
    const role = wx.getStorageSync(springAuth.KEYS.ROLE);
    if (!pagePermission.canAccessMiniPage('/package-supplies/pages/suppliesMine/index', role, 'STAFF')) {
      wx.showToast({ title: '无权限', icon: 'none' });
      return;
    }
    const proceed = () => {
  ...
  goProcess() {
    ...
    const proceed = () => {
      this._navOutToSubPage = true;
      wx.navigateTo({ url: '/package-supplies/pages/suppliesProcess/index' });
    };
    if (String(this.data.claimReviseOrderId || '').trim()) {
      this._confirmExitRevise(proceed);
      return;
  ...
  goAdmin() {
    ...
    const proceed = () => {
      this._navOutToSubPage = true;
      wx.navigateTo({ url: '/package-supplies/pages/suppliesAdmin/index' });
    };
    if (String(this.data.claimReviseOrderId || '').trim()) {
      this._confirmExitRevise(proceed);
      return;
