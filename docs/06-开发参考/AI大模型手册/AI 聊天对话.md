...
④ `knowledge_ids` 字段：关联的知识库编号数组，对应 `ai_knowledge` 表的 `id` 字段。后续使用，可见 [《AI 知识库》](/开发参考/AI大模型手册/AI 知识库（RAG）.md) 文档。

⑤ `tool_ids` 字段：关联的工具编号数组，对应 `ai_tool` 表的 `id` 字段。后续使用，可见 [《AI 工具库》](/开发参考/AI大模型手册/AI 工具（function calling）.md) 文档。

⑥ `mcp_client_names` 字段：引用的 MCP Client 名字列表。后续使用，可见 [《MCP Client 客户端》](/开发参考/AI大模型手册/MCP Client 客户端.md) 文档。
...
`reason_content` 字段：推理内容，对应支持推理模式（thinking）的模型，模型在输出最终回答之前，先输出的思维链内容。对应 [《推理模式（thinking）》](/开发参考/AI大模型手册/推理模式（thinking）.md) 文档。

⑤ `use_context` 字段：表示是否携带上下文。如果为 `true` 时，表示这条消息会携带上下文，此时调用模型时，会带该会话的最后 `max_contexts` 组消息。

注意，它是在消息发送框的左下角的蓝色 switch 【上下文】小按钮噢！

⑥ `segment_ids` 字段：知识库找回的段落编号数组，对应 `ai_knowledge_segment` 表的 `id` 字段，可见 [《AI 知识库》](/开发参考/AI大模型手册/AI 知识库（RAG）.md) 文档。

⑦ `web_search_pages` 字段：联网搜索的网页内容数组，对应 [《AI 联网搜索》](/开发参考/AI大模型手册/联网搜索.md) 文档。
