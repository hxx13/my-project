...
项目基于 Spring AI 提供的 `spring-ai-deepseek`，实现 DeepSeek 的接入：
...
模型有：`deepseek-chat`、`deepseek-reasoner` 等等，可通过 《DeepSeek 首次调用 API 》 查看。
...
① 参考 [《【模型接入】通义千问》](/开发参考/AI大模型手册/【模型接入】通义千问.md) 文档，申请阿里云密钥，并配置 API 密钥。
...
① 参考 [《【模型接入】字节豆包》](/开发参考/AI大模型手册/【模型接入】字节豆包.md) 文档，申请字节密钥，并配置 API 密钥。
...
① 参考 [《【模型接入】硅基流动》](/开发参考/AI大模型手册/【模型接入】硅基流动.md) 文档，申请硅基密钥，并配置 API 密钥。
