...
回款模块，由 `yudao-module-crm` 后端模块的 `receivable` 包实现，已经和 [BPM 工作流](/开发参考/支付手册/功能开启.md) 打通审批。
...
④ `owner_user_id` 字段：回款的负责人编号，和线索类似。不重复赘述，详细可见 [《【通用】数据权限》](/开发参考/CRM手册/【通用】数据权限.md) 文档。

⑤ `process_instance_id` 字段：工作流编号，用于和 [BPM 工作流](/开发参考/支付手册/功能开启.md) 打通审批。提交审批后，会自动创建一个工作流实例，并记录到该字段。
...
① 参考 [《【合同】合同管理、合同提醒》](/开发参考/CRM手册/【合同】合同管理、合同提醒.md) 文档，创建一个合同。注意，必须审核通过，才能进行回款。
...
如果不了解怎么配置的同学，可以学习下 [《BPM 工作流》](/开发参考/支付手册/功能开启.md) 的文档。
...
③ `owner_user_id` 字段：回款计划的负责人编号，和线索类似。不重复赘述，详细可见 [《【通用】数据权限》](/开发参考/CRM手册/【通用】数据权限.md) 文档。
