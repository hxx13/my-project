...
本文接 [《【线索】线索管理》](/开发参考/CRM手册/【线索】线索管理.md)，主要讲解四部分：客户信息、客户公海配置、客户限制配置、联系人。它们的表关系如下图所示：
...
① `owner_user_id` 字段：客户的负责人编号，和线索类似。不重复赘述，详细可见 [《【通用】数据权限》](/开发参考/CRM手册/【通用】数据权限.md) 文档。

差别在多了 `owner_time` 字段，成为负责人超过一定时间后，客户自动释放进入公海。当然，如果客户被 `lock_status` 锁定，或者有 `deal_status` 成交，就不会释放。

② 从 `follow_up_status` 到 `contact_next_time` 字段：跟进相关，和线索类似。不重复赘述，详细可见 [《【通用】跟进记录》](/开发参考/CRM手册/【通用】跟进记录、待办事项.md) 文档。
...
目前，该功能是通过 CrmCustomerAutoPutPoolJob 定时任务实现的。你可以参考 [《定时任务》](/开发参考/中间件手册/定时任务.md) 文档，自己配置每 5 分钟执行一次。
...
② `owner_user_id` 字段：联系人的负责人编号，和线索类似。不重复赘述，详细可见 [《【通用】数据权限》](/开发参考/CRM手册/【通用】数据权限.md) 文档。

③ 从 `contact_last_time` 到 `contact_next_time` 字段：跟进相关，和线索类似。不重复赘述，详细可见 [《【通用】跟进记录》](/开发参考/CRM手册/【通用】跟进记录、待办事项.md) 文档。
