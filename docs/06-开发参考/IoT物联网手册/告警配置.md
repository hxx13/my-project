...
它需要和 [《场景联动》](/开发参考/IoT物联网手册/场景联动.md) 配合使用：场景联动规则中的"告警触发"动作会根据告警配置创建告警记录，"告警恢复"动作会批量处理未处理的告警记录。关于告警触发和告警恢复动作，详见 [《场景联动》](/开发参考/IoT物联网手册/场景联动.md) 的「1.1.2 动作（Action）」小节。
...
以内置的 id 为 25 的 演示设备 为例，演示如何配置告警：当设备上报的 `height` 属性大于 2000 时触发告警，并通知管理员。
