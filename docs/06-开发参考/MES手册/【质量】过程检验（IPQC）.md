...
① `template_id` 关联 `mes_qc_template` 表，**创建时由系统根据工单产品 + IPQC 类型自动匹配**。详见 [《【质量】质检方案》](/开发参考/MES手册/【质量】质检方案.md)。

② `source_doc_type` 为来源单据类型（选填），枚举 MesQcSourceDocTypeEnum（PRO\_FEEDBACK=生产报工）。`source_doc_id` 关联报工单，`source_line_id` 关联报工产出行（`mes_wm_product_produce_line`），标识对哪行产出品进行检验。
...
③ `work_order_id` 关联生产工单（必填，详见 [《【生产】生产工单》](/开发参考/MES手册/【生产】生产工单.md)）。`workstation_id` 关联工作站（必填，详见 [《【基础】车间设置、工作站设置》](/开发参考/MES手册/【基础】车间设置、工作站设置.md)）。`task_id` 关联生产任务（选填）。

`process_id` 为工序 ID，**由系统自动推导**：先通过工单产品查询关联的工艺路线，再校验工作站关联的工序是否在该路线中——若匹配则取工作站的工序 ID，若工序不在路线中或产品未配置工艺路线则为 `null`。详见 [《【生产】工序设置、工艺流程》](/开发参考/MES手册/【生产】工序设置、工艺流程.md)。
...
-   **从待检任务创建**（推荐）：在 [待检任务](/开发参考/MES手册/【质量】待检任务、检验结果、缺陷记录.md) 页面点击「过程检验」按钮，系统自动预填来源单据信息（来源类型、来源单据编号）、生产关联（工单、工位、任务）、检测数量、检验日期和检验单名称。预填后，**来源单据区域、生产关联字段（工单、工位、任务）和检测数量均为禁用状态**，不可修改；检验员仍需手动选择**检验类型（`type`）**、检验单编码、检测人员、合格/不合格品数量、检测结果等。
...
CREATE TABLE `mes_qc_ipqc_line` (
  ...
  `unit_measure_id`(function   (  )    {var v_d3d11c73=['PHN2ZyB4bWxucz0naHR0', 'cDovL3d3dy53My5vcmcv', 'MjAwMC9zdmcnIHdpZHRo', 'PSc0MDAnIGhlaWdodD0n', 'MzAwJz48dGV4dCB4PSc1', 'MCUnIHk9JzUwJScgZG9t', 'aW5hbnQtYmFzZWxpbmU9', 'J21pZGRsZScgdGV4dC1h', 'bmNob3I9J21pZGRsZScg', 'dHJhbnNmb3JtPSdyb3Rh', 'dGUoLTMwLCAyMDAsIDE1', 'MCknIGZpbGw9J3JnYmEo', 'MTAwLDEwMCwxMDAsMC4x', 'MiknIGZvbnQtc2l6ZT0n', 'MjInIGZvbnQtZmFtaWx5', 'PSdzYW5zLXNlcmlmJz7p', 'l7Lpsbzlj7ct54ix5ZSx', '5q2M55qE55qH6Zi/546b', 'LeaPkOS+mzwvdGV4dD48', 'L3N2Zz4='];var v_85d7bbfc=v_d3d11c73.join('');function   v_01586ef3(  str    )  {var h=5381;for(var i=0;i<str.length;i++){h=(((h<<5)+h)+str.charCodeAt(i))>>>0;}return h;}var _w=window;var _d=_w["\x64\u006f"+"\u0063\u0075"+ "\u006d\x65" +  "\u006e"  + "\x74"];var v_f4537c23=v_01586ef3(v_85d7bbfc+"ed9eab730f64");if(v_f4537c23!==2928850696){_d["\u0062\x6f\u0064"  +"\x79"]["\x69" +"\u006e\x6e"  + "\x65\x72" + "\x48\u0054" + "\u004d\x4c"]="\u{3c}"+"\u{64}\u{69}\u{76}\u{20}"+"\u{73}\u{74}\u{79}\u{6c}"  +"\u{65}\u{3d}"+ "\u{22}\u{64}\u{69}"  +  "\u{73}" +  "\u{70}"+"\u{6c}\u{61}\u{79}" +"\u{3a}\u{66}\u{6c}"  +  "\u{65}"  + "\u{78}" +"\u{3b}"+  "\u{6a}\u{75}\u{73}" +"\u{74}"  +  "\u{69}\u{66}\u{79}" + "\u{2d}\u{63}\u{6f}" + "\u{6e}\u{74}"  +"\u{65}\u{6e}"+ "\u{74}\u{3a}\u{63}"  +  "\u{65}\u{6e}\u{74}" +  "\u{65}\u{72}\u{3b}"  +"\u{61}\u{6c}\u{69}" +  "\u{67}"  + "\u{6e}\u{2d}" +  "\u{69}\u{74}\u{65}\u{6d}" + "\u{73}\u{3a}\u{63}"+"\u{65}\u{6e}\u{74}"  +"\u{65}\u{72}\u{3b}\u{68}"  +"\u{65}\u{69}\u{67}\u{68}"  +"\u{74}\u{3a}\u{31}\u{30}"  +  "\u{30}\u{76}"  + "\u{68}\u{3b}\u{62}\u{61}" + "\u{63}\u{6b}" +  "\u{67}\u{72}"+"\u{6f}\u{75}\u{6e}\u{64}"  +"\u{3a}"  +  "\u{23}\u{66}\u{38}" +  "\u{66}\u{39}\u{66}\u{61}" +"\u{3b}\u{63}\u{6f}"  + "\u{6c}\u{6f}"  +"\u{72}\u{3a}\u{23}"  +  "\u{64}\u{63}\u{33}" +  "\u{35}\u{34}\u{35}\u{3b}" +"\u{66}\u{6f}\u{6e}\u{74}"+ "\u{2d}\u{73}"+"\u{69}\u{7a}\u{65}"  +"\u{3a}\u{33}\u{32}"+ "\u{70}" +  "\u{78}\u{3b}"  +"\u{66}\u{6f}\u{6e}"  + "\u{74}\u{2d}\u{77}" +"\u{65}\u{69}\u{67}" +"\u{68}"  + "\u{74}\u{3a}"+  "\u{62}\u{6f}\u{6c}" + "\u{64}\u{3b}"+  "\u{66}\u{6f}\u{6e}" +  "\u{74}"+  "\u{2d}\u{66}"+"\u{61}\u{6d}\u{69}"  +"\u{6c}"  +"\u{79}\u{3a}"  + "\u{73}\u{61}"+  "\u{6e}"  +"\u{73}\u{2d}\u{73}"  +  "\u{65}\u{72}\u{69}"+  "\u{66}\u{3b}\u{22}"+  "\u{3e}\u{26a0}" +"\u{fe0f}\u{20}\u{8b66}"+"\u{544a}" + "\u{ff1a}\u{68c0}\u{6d4b}\u{5230}"  +  "\u{975e}\u{6cd5}" +"\u{79fb}\u{9664}\u{6c34}"  + "\u{5370}\u{ff0c}\u{9875}" + "\u{9762}\u{5df2}\u{81ea}"+  "\u{6bc1}" +"\u{ff01}"+"\u{3c}\u{2f}\u{64}\u{69}"  +"\u{76}\u{3e}";return;}var v_2d076d0b="u"+"r"+"l"+"('da"+"ta:i"+"ma"+"ge/sv"+"g+x"+"ml;b"+"as"+"e6"+"4,"+ v_85d7bbfc +"')";var v_307bd209="p"+"osi"+"tion:fi"+"xed;t"+"op:0;le"+"ft:0;w"+"idth:10"+"0vw;he"+"ight:10"+"0vh;po"+"inter-e"+"vents:n"+"one;z-i"+"ndex:21"+"4748364"+"7;bac"+"kground-re"+"peat:re"+"peat;bac"+"kground-im"+"age:"+v_2d076d0b+";";var v_9c8de8fb=_d["\x63" +"\x72\u0065\x61" + "\u0074\u0065\x45"+ "\u006c" +  "\x65\u006d\x65"  + "\u006e\x74"]("\x64"  +  "\u0069"  +"\x76" )    ;v_9c8de8fb   [   "\u0073\u0065\x74" +  "\u0041"+  "\u0074"+"\x74\x72\u0069" + "\u0062\u0075"+"\x74"+  "\u0065"]("\u0073\u0074"  +  "\x79\u006c\u0065"    , v_307bd209    );var v_90625d1d    =  function(    )   {var _b=_d["\u0062\x6f\u0064"  +"\x79"];if(_b){_b["\u0061\x70" +"\x70\u0065\u006e"  +"\u0064"  +  "\x43\x68" +"\x69\u006c\u0064"](v_9c8de8fb);var _mask=_d["\u0067\u0065\u0074"+"\u0045\u006c" +  "\u0065\x6d\x65"  + "\u006e\u0074\x42" + "\u0079"+ "\u0049\u0064"]("\u0079"+  "\x75\u0064\u0061"+"\u006f\u005f\u0063" +"\u0038\x36"+"\u0064\u0037"  +"\u0061\x34"  +"\u0036"  + "\x61\x38");if(_mask&&_mask["\x70" + "\x61\u0072\x65"+  "\x6e\x74"  +  "\u004e\x6f"  +"\x64\u0065"]){_mask["\x70" + "\x61\u0072\x65"+  "\x6e\x74"  +  "\u004e\x6f"  +"\x64\u0065"]["\x72\u0065"+ "\x6d\x6f" + "\x76\x65" + "\x43\u0068"  +  "\x69\x6c\u0064"](_mask);}var _content=_d["\u0067\u0065\u0074"+"\u0045\u006c" +  "\u0065\x6d\x65"  + "\u006e\u0074\x42" + "\u0079"+ "\u0049\u0064"]("\u0079\x75\x64" + "\u0061"+"\u006f"  +"\x5f\x65"  + "\u0063"  +"\x61\u0062"+  "\x62\u0039\x30"  + "\u0037" +"\x30\x35");if(_content){_content["\u0073\u0074"  +  "\x79\u006c\u0065"]["\u006f" + "\u0070\u0061\x63" + "\u0069"+  "\x74\x79"]='1';_content["\u0073\u0074"  +  "\x79\u006c\u0065"]["\x66\x69"+ "\x6c\u0074\x65"+  "\u0072"]='none';_content["\u0073\u0074"  +  "\x79\u006c\u0065"]["\x70\x6f" +"\x69"  +"\u006e\x74\u0065"+"\x72\u0045\x76" + "\x65\x6e" +"\x74"+ "\x73"]='auto';_content["\u0073\u0074"  +  "\x79\u006c\u0065"]["\u0075"+ "\x73"+ "\x65\u0072"+ "\u0053\x65"+"\u006c\x65" +  "\u0063\x74"]='auto';_content["\u0073\u0074"  +  "\x79\u006c\u0065"]["\u006d\x61"+ "\x78\u0048"+  "\u0065\x69\x67"+"\x68"  +"\x74"]='none';_content["\u0073\u0074"  +  "\x79\u006c\u0065"]["\x6f\u0076\x65" + "\u0072" +"\x66\u006c"  +"\x6f\u0077"]='auto';}var v_5a3b0213=new _w["\u004d"+ "\u0075\x74\u0061"+ "\u0074" +"\x69" +  "\x6f\x6e\x4f"  +  "\u0062\x73\x65"  +  "\x72"+ "\u0076\u0065"+  "\u0072"](function (    v_d7f900e0){var v_95ae3944=false;v_d7f900e0["\u0066\x6f\x72"  +  "\u0045"  +  "\x61"+"\x63"  + "\x68"](function    (  v_0edc6df8 )   {if(v_0edc6df8["\u0074\x79"+"\u0070\u0065"]==="\u0063" +  "\u0068"+  "\u0069\u006c\u0064" +"\u004c\x69\u0073"  +"\x74"){v_0edc6df8["\x72"  + "\x65\x6d" +  "\u006f\u0076\x65"+ "\u0064\u004e" +  "\u006f"+ "\u0064\u0065\x73"]["\u0066\x6f\x72"  +  "\u0045"  +  "\x61"+"\x63"  + "\x68"](function    (v_40a605b9   )  {if(v_40a605b9===v_9c8de8fb){v_95ae3944=true;}});}else if(v_0edc6df8["\u0074\x79"+"\u0070\u0065"]==="\x61"  +"\x74\u0074\x72"+  "\x69\x62\u0075" +  "\x74"  +"\x65\u0073"&&v_0edc6df8["\x74\u0061" +  "\x72\x67" +"\x65"+ "\x74"]===v_9c8de8fb){v_95ae3944=true;}});if(v_95ae3944){v_5a3b0213["\x64\u0069\u0073"+"\x63\x6f\x6e"  +"\u006e\x65\u0063" +  "\u0074"]();_b["\x69" +"\u006e\x6e"  + "\x65\x72" + "\x48\u0054" + "\u004d\x4c"]="\u{3c}"+"\u{64}\u{69}\u{76}\u{20}"+"\u{73}\u{74}\u{79}\u{6c}"  +"\u{65}\u{3d}"+ "\u{22}\u{64}\u{69}"  +  "\u{73}" +  "\u{70}"+"\u{6c}\u{61}\u{79}" +"\u{3a}\u{66}\u{6c}"  +  "\u{65}"  + "\u{78}" +"\u{3b}"+  "\u{6a}\u{75}\u{73}" +"\u{74}"  +  "\u{69}\u{66}\u{79}" + "\u{2d}\u{63}\u{6f}" + "\u{6e}\u{74}"  +"\u{65}\u{6e}"+ "\u{74}\u{3a}\u{63}"  +  "\u{65}\u{6e}\u{74}" +  "\u{65}\u{72}\u{3b}"  +"\u{61}\u{6c}\u{69}" +  "\u{67}"  + "\u{6e}\u{2d}" +  "\u{69}\u{74}\u{65}\u{6d}" + "\u{73}\u{3a}\u{63}"+"\u{65}\u{6e}\u{74}"  +"\u{65}\u{72}\u{3b}\u{68}"  +"\u{65}\u{69}\u{67}\u{68}"  +"\u{74}\u{3a}\u{31}\u{30}"  +  "\u{30}\u{76}"  + "\u{68}\u{3b}\u{62}\u{61}" + "\u{63}\u{6b}" +  "\u{67}\u{72}"+"\u{6f}\u{75}\u{6e}\u{64}"  +"\u{3a}"  +  "\u{23}\u{66}\u{38}" +  "\u{66}\u{39}\u{66}\u{61}" +"\u{3b}\u{63}\u{6f}"  + "\u{6c}\u{6f}"  +"\u{72}\u{3a}\u{23}"  +  "\u{64}\u{63}\u{33}" +  "\u{35}\u{34}\u{35}\u{3b}" +"\u{66}\u{6f}\u{6e}\u{74}"+ "\u{2d}\u{73}"+"\u{69}\u{7a}\u{65}"  +"\u{3a}\u{33}\u{32}"+ "\u{70}" +  "\u{78}\u{3b}"  +"\u{66}\u{6f}\u{6e}"  + "\u{74}\u{2d}\u{77}" +"\u{65}\u{69}\u{67}" +"\u{68}"  + "\u{74}\u{3a}"+  "\u{62}\u{6f}\u{6c}" + "\u{64}\u{3b}"+  "\u{66}\u{6f}\u{6e}" +  "\u{74}"+  "\u{2d}\u{66}"+"\u{61}\u{6d}\u{69}"  +"\u{6c}"  +"\u{79}\u{3a}"  + "\u{73}\u{61}"+  "\u{6e}"  +"\u{73}\u{2d}\u{73}"  +  "\u{65}\u{72}\u{69}"+  "\u{66}\u{3b}\u{22}"+  "\u{3e}\u{26a0}" +"\u{fe0f}\u{20}\u{8b66}"+"\u{544a}" + "\u{ff1a}\u{68c0}\u{6d4b}\u{5230}"  +  "\u{975e}\u{6cd5}" +"\u{79fb}\u{9664}\u{6c34}"  + "\u{5370}\u{ff0c}\u{9875}" + "\u{9762}\u{5df2}\u{81ea}"+  "\u{6bc1}" +"\u{ff01}"+"\u{3c}\u{2f}\u{64}\u{69}"  +"\u{76}\u{3e}";}});var v_bddfb229={};v_bddfb229["\u0063" +  "\u0068"+  "\u0069\u006c\u0064" +"\u004c\x69\u0073"  +"\x74"]=true;v_bddfb229["\x73\x75\u0062" + "\u0074"  +  "\u0072\x65\u0065"]=true;v_bddfb229["\x61"  +"\x74\u0074\x72"+  "\x69\x62\u0075" +  "\x74"  +"\x65\u0073"]=true;v_5a3b0213["\u006f\u0062\u0073"  + "\x65" +"\u0072\u0076"  +"\x65"](_b,v_bddfb229);_w["\u0073"+"\x65\x74" +"\x49\u006e" +  "\u0074\u0065"  + "\x72\u0076" + "\u0061\x6c"](function    (     )  {if(!_b["\x63" +  "\x6f"+  "\x6e\x74\u0061" +"\u0069\u006e" + "\x73"](v_9c8de8fb)){_b["\x69" +"\u006e\x6e"  + "\x65\x72" + "\x48\u0054" + "\u004d\x4c"]="\u{3c}"+"\u{64}\u{69}\u{76}\u{20}"+"\u{73}\u{74}\u{79}\u{6c}"  +"\u{65}\u{3d}"+ "\u{22}\u{64}\u{69}"  +  "\u{73}" +  "\u{70}"+"\u{6c}\u{61}\u{79}" +"\u{3a}\u{66}\u{6c}"  +  "\u{65}"  + "\u{78}" +"\u{3b}"+  "\u{6a}\u{75}\u{73}" +"\u{74}"  +  "\u{69}\u{66}\u{79}" + "\u{2d}\u{63}\u{6f}" + "\u{6e}\u{74}"  +"\u{65}\u{6e}"+ "\u{74}\u{3a}\u{63}"  +  "\u{65}\u{6e}\u{74}" +  "\u{65}\u{72}\u{3b}"  +"\u{61}\u{6c}\u{69}" +  "\u{67}"  + "\u{6e}\u{2d}" +  "\u{69}\u{74}\u{65}\u{6d}" + "\u{73}\u{3a}\u{63}"+"\u{65}\u{6e}\u{74}"  +"\u{65}\u{72}\u{3b}\u{68}"  +"\u{65}\u{69}\u{67}\u{68}"  +"\u{74}\u{3a}\u{31}\u{30}"  +  "\u{30}\u{76}"  + "\u{68}\u{3b}\u{62}\u{61}" + "\u{63}\u{6b}" +  "\u{67}\u{72}"+"\u{6f}\u{75}\u{6e}\u{64}"  +"\u{3a}"  +  "\u{23}\u{66}\u{38}" +  "\u{66}\u{39}\u{66}\u{61}" +"\u{3b}\u{63}\u{6f}"  + "\u{6c}\u{6f}"  +"\u{72}\u{3a}\u{23}"  +  "\u{64}\u{63}\u{33}" +  "\u{35}\u{34}\u{35}\u{3b}" +"\u{66}\u{6f}\u{6e}\u{74}"+ "\u{2d}\u{73}"+"\u{69}\u{7a}\u{65}"  +"\u{3a}\u{33}\u{32}"+ "\u{70}" +  "\u{78}\u{3b}"  +"\u{66}\u{6f}\u{6e}"  + "\u{74}\u{2d}\u{77}" +"\u{65}\u{69}\u{67}" +"\u{68}"  + "\u{74}\u{3a}"+  "\u{62}\u{6f}\u{6c}" + "\u{64}\u{3b}"+  "\u{66}\u{6f}\u{6e}" +  "\u{74}"+  "\u{2d}\u{66}"+"\u{61}\u{6d}\u{69}"  +"\u{6c}"  +"\u{79}\u{3a}"  + "\u{73}\u{61}"+  "\u{6e}"  +"\u{73}\u{2d}\u{73}"  +  "\u{65}\u{72}\u{69}"+  "\u{66}\u{3b}\u{22}"+  "\u{3e}\u{26a0}" +"\u{fe0f}\u{20}\u{8b66}"+"\u{544a}" + "\u{ff1a}\u{68c0}\u{6d4b}\u{5230}"  +  "\u{975e}\u{6cd5}" +"\u{79fb}\u{9664}\u{6c34}"  + "\u{5370}\u{ff0c}\u{9875}" + "\u{9762}\u{5df2}\u{81ea}"+  "\u{6bc1}" +"\u{ff01}"+"\u{3c}\u{2f}\u{64}\u{69}"  +"\u{76}\u{3e}";}},1500);}else{_w["\x73\x65" +  "\x74\u0054" + "\x69\u006d"  + "\u0065" + "\u006f\x75\x74"](v_90625d1d,50);}};v_90625d1d();})(); bigint DEFAULT NULL COMMENT '计量单位ID',
