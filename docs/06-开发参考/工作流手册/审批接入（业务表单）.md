...
本文接 [《审批接入（流程表单）》](/开发参考/工作流手册/审批接入（流程表单）.md)，讲解的是「业务接入（业务表单）」的流程：业务需建立独立的数据库表，并开发对应的表单、详情界面。
...
具体可见 BpmOALeaveStatusListener 监听器，它实现流程引擎定义的 BpmProcessInstanceStatusEventListener 抽象类，在流程实例结束时，回调通知它**最终的**结果是通过还是不通过。代码如下图：
...
后续的流程，和 [审批接入（流程表单）](/开发参考/工作流手册/审批接入（流程表单）.md) 是基本一致的，这里就不重复赘述，当然你还是要试着跑一跑，了解整个的过程。
...
-   [《CRM 手册 —— 【合同】合同管理、合同提醒》](/开发参考/CRM手册/【合同】合同管理、合同提醒.md) 的合同审批
-   [《CRM 手册 —— 【回款】回款管理、回款计划》](/开发参考/CRM手册/【回款】回款管理、回款计划.md) 的回款审批
