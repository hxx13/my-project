...
- **最后更新**: 2026-06-10T23:00:00+08:00
- **状态**: 进行中
- **当前 Phase**: Phase 3 编码实现（3/4，计划已完成，待执行）
...
- **进展到哪一步**：设计完全完成、计划已写、UI 方向已定，**准备开始写代码**
- **卡在哪里**：无阻塞
- **等待用户确认的事项**：
  - 统计卡片缩小后的尺寸（建议高度从 ~100px 降到 ~60px，字号从 28px 降到 20px）
  - 是否立即开始编码

### ⚠️ 计划修正清单（编码前必读）

实施计划在 UI 设计之前写的，有几处**必须在编码前修正**：

| # | 修正项 | 原因 |
|---|--------|------|
| 1 | 所有组件硬编码颜色 → 替换为 `--app-*` 语义令牌引用 | 违反 UI 规范"原则一：令牌是唯一真相源" |
| 2 | `smartsheet-theme.css` 自定义变量 → 改为组件令牌 `--smartsheet-*`，引用 `--app-*` 语义层 | 违反令牌分层架构 |
| 3 | 暗色 `#09090b` → 对齐 Bento 暖黑 `#12100E` 或项目 `--color-slate-950` | Bento 设计方向确认 |
| 4 | Z-index `z-50` → `var(--z-dropdown)` / `var(--z-modal)` 等 | 违反 Z-Index 层级表 |
| 5 | Bento 强调色 `#80A1C1` → 对齐项目 `--color-blue-*` 体系或注册为 `--color-blue-*` 扩展 | 令牌体系统一 |
| 6 | 顶部统计卡片高度缩减 | 用户反馈 |
...
1. **打开实施计划**：`docs/superpowers/plans/2026-06-10-smartsheet-plan.md`
2. **先修正计划中的令牌问题**：按上面"计划修正清单"逐条更新 Task 10（Theme CSS）、Task 11-14（组件 TSX）的代码示例，将硬编码颜色替换为 `--app-*` 令牌引用
3. **按 Task 1 → 16 顺序执行**：SQL 迁移 → 后端 Entity/Mapper/Service/Controller → 前端 Types/API/Hook/组件/路由
4. **统计卡片实现时**：高度 ~60px、字号 20px、仅保留核心指标（行数/列数/填写率），不占过多垂直空间
5. **Bento 风格落实**：
   - 页面底色 `var(--app-color-surface-page)` 覆盖为暖米白 `#FFF5E6`（亮色）/ 暖黑 `#12100E`（暗色）——或新增 `.theme-bento` 主题
   - 所有卡片 `border-radius: 14-20px`
   - 行头底色用暖桃弱调 `rgba(250,212,192,0.15)` 替代纯灰
   - 选中态用 `box-shadow` 发光环替代硬描边
