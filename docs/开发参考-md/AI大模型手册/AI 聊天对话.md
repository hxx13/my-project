...
一般情况下，建议先使用 《DeepSeek》 模块，因为免费送了一些 tokens，可以先体验一下。
...
④ `knowledge_ids` 字段：关联的知识库编号数组，对应 `ai_knowledge` 表的 `id` 字段。后续使用，可见 《AI 知识库》 文档。

⑤ `tool_ids` 字段：关联的工具编号数组，对应 `ai_tool` 表的 `id` 字段。后续使用，可见 《AI 工具库》 文档。

⑥ `mcp_client_names` 字段：引用的 MCP Client 名字列表。后续使用，可见 《MCP Client 客户端》 文档。

###  2.2 管理后台

① 前端对应 [AI 大模型 -> 控制台 -> 聊天角色] 菜单，对应 `yudao-ui-admin-vue3` 项目的 `@/views/ai/model/chatRole` 目录，提供给管理员使用，创建公开角色。

![聊天角色](/knowledge-images/fbff736885ea.png)
...
![角色仓库](/knowledge-images/704e29b7f580.png)
...
![修改模型](/knowledge-images/bff9aa27e70e.png)
...
`reason_content` 字段：推理内容，对应支持推理模式（thinking）的模型，模型在输出最终回答之前，先输出的思维链内容。对应 《推理模式（thinking）》 文档。

⑤ `use_context` 字段：表示是否携带上下文。如果为 `true` 时，表示这条消息会携带上下文，此时调用模型时，会带该会话的最后 `max_contexts` 组消息。

注意，它是在消息发送框的左下角的蓝色 switch 【上下文】小按钮噢！

⑥ `segment_ids` 字段：知识库找回的段落编号数组，对应 `ai_knowledge_segment` 表的 `id` 字段，可见 《AI 知识库》 文档。

⑦ `web_search_pages` 字段：联网搜索的网页内容数组，对应 《AI 联网搜索》 文档。
...
![聊天管理](/knowledge-images/1fe9e1f4d5d5.png)
