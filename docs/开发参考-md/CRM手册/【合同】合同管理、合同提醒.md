...
合同模块，由 `yudao-module-crm` 后端模块的 `contract` 包实现，已经和 BPM 工作流 打通审批。

##  1. 合同

合同，由 `yudao-module-crm` 后端模块的 `business` 包的 CrmContractController 实现。表关系如下：

![表关系](/knowledge-images/c2327d07d9dc.png)
