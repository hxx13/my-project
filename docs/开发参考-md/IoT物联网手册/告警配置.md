...
它需要和 《场景联动》 配合使用：场景联动规则中的"告警触发"动作会根据告警配置创建告警记录，"告警恢复"动作会批量处理未处理的告警记录。关于告警触发和告警恢复动作，详见 《场景联动》 的「1.1.2 动作（Action）」小节。

##  1. 告警

告警由两部分组成：**告警配置**（定义告警规则和通知方式）和**告警记录**（保存每次告警的详细信息）。

![表关系](/knowledge-images/fadc4c85ab12.png)
...
![管理后台 - 告警配置列表](/knowledge-images/2d160c0747b7.png)

② 点击【新增】按钮，弹出新增告警配置对话框。

![管理后台 - 创建告警配置](/knowledge-images/a4439b2e0190.png)
...
![管理后台 - 告警记录列表](/knowledge-images/cd7f1c659e31.png)

② 点击操作列的【处理】按钮，弹出输入框输入处理原因，即可将告警标记为已处理。

![管理后台 - 处理告警](/knowledge-images/271c85d7fd85.png)
...
![快速上手 - 场景联动规则](/knowledge-images/4e313c8546f4.png)
...
![快速上手 - 告警配置示例](/knowledge-images/625bf8dacf3a.png)
...
![快速上手 - 模拟设备上报属性](/knowledge-images/0a640dd36a68.png)

② 进入 [IoT 物联网 -> 告警中心 -> 告警记录]，可以看到新生成了一条告警记录，告警级别为 ERROR，说明告警触发成功。

![快速上手 - 告警记录](/knowledge-images/3c05ff1a2de2.png)
