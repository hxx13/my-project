...
- `alertConfigId`：告警配置编号，用于告警触发/恢复动作。关于告警触发和告警恢复，详见 《告警配置》。

###  1.2 管理后台（列表）

对应 [IoT 物联网 -> 规则引擎 -> 场景联动] 菜单，对应前端项目的 `@/views/iot/rule/scene` 目录。

![管理后台 - 场景联动列表](/knowledge-images/8a1e3cf776f5.png)

###  1.3 管理后台（创建/编辑）

点击【新增】按钮，右侧弹出抽屉表单。表单分为三个区域：基础信息、触发器配置、执行动作配置。

![管理后台 - 创建场景联动](/knowledge-images/784453891839.png)
...
![管理后台 - 设备触发器配置](/knowledge-images/91273cb9e6e4.png)

---

② **定时触发**：输入 CRON 表达式，同样可以添加执行条件组。

![管理后台 - 定时触发器配置](/knowledge-images/7fb365477e6a.png)
...
![管理后台 - 执行动作配置](/knowledge-images/b08a89fa1d5d.png)
...
![快速上手 - 配置示例](/knowledge-images/d6a6f0cdbe2c.png)
...
![快速上手 - 模拟设备上报属性](/knowledge-images/12f5d115fcf9.png)

② 在设备详情的「设备消息」Tab，可以看到触发了一条"服务调用"的下行消息，说明场景联动执行成功。

![快速上手 - 设备消息](/knowledge-images/f8b23105b6b6.png)
