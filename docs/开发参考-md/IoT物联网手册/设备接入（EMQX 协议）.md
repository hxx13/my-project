...
![EMQX 登录页](/knowledge-images/561d12d5dffa.png)
...
![客户端认证列表](/knowledge-images/19079168f65e.png)

② 认证方式选择 **Password-Based**，点击「下一步」：

![选择认证方式](/knowledge-images/6e75dd409d9e.png)

③ 数据源选择「**内置数据库**」，点击「下一步」：

![选择数据源 - 内置数据库](/knowledge-images/1cb6abafe558.png)

④ 配置参数（账号类型 `username`、加密方式 `sha256`），点击「创建」：

![配置内置数据库参数](/knowledge-images/e77ccf47f9fa.png)
...
![内置数据库认证已创建](/knowledge-images/5610bc12baa2.png)

![用户管理页面](/knowledge-images/2356cc76c635.png)
...
![客户端认证列表 - 再次创建](/knowledge-images/3e4bf7e5bfd4.png)

② 认证方式选择 **Password-Based**，点击「下一步」：

![选择认证方式](/knowledge-images/c53df50e9006.png)

③ 数据源选择「**HTTP 服务**」（内置数据库已标记"已添加"），点击「下一步」：

![选择数据源 - HTTP 服务](/knowledge-images/8c629b69344f.png)
...
![配置 HTTP 认证参数](/knowledge-images/939afc70f89a.png)

⑤ 创建完成后，可以看到客户端认证列表中同时存在「内置数据库」和「HTTP 服务」两条认证：

![两条认证创建完成](/knowledge-images/d66cf11bc24e.png)
...
![Webhook 列表](/knowledge-images/f28f4774b371.png)

② 触发器选择「事件」，勾选 **连接建立** 和 **连接断开**；URL 填写事件地址，点击「保存」：

- 事件地址：`http://{网关IP}:8090/mqtt/event`

![创建 Webhook](/knowledge-images/16925effe82c.png)
...
EMQX 协议的主题格式、消息格式、认证方式与 内置 MQTT 协议 完全一致，设备端使用方式无差异，只需将 MQTT 连接地址指向 EMQX Broker（而非网关内置 MQTT 端口）。
...
![MQTTX 连接配置](/knowledge-images/2d9898c44e3d.png)
...
![设备在线状态](/knowledge-images/cc1d229648a0.png)
...
![MQTTX 属性上报](/knowledge-images/95557febbd74.png)
...
![属性数据](/knowledge-images/c74de6d14deb.png)

![属性消息日志](/knowledge-images/57ea88f8714e.png)
...
![MQTTX 事件上报](/knowledge-images/9017dbfb2abc.png)
...
![事件数据](/knowledge-images/210e730e626d.png)

![事件消息日志](/knowledge-images/fa0912f5854b.png)
