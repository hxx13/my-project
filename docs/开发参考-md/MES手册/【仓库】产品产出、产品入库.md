...
![产品产出入库模块](/knowledge-images/65a4f3bc1164.svg)
...
产品产出没有独立的前端入口页面，属于**系统自动触发**的后台模块。当报工审批通过后，系统根据报工数量和质检结果自动生成产出记录并入库到虚拟线边库。详见 《【生产】生产报工》。
...
① `work_order_id` 关联 `mes_pro_work_order` 表的 `id` 字段，详见 《【生产】生产工单》。

`task_id` 关联 `mes_pro_task` 表，详见 《【生产】生产排产、工序流转卡》。

`workstation_id` 关联 `mes_md_workstation` 表，详见 《【基础】车间设置、工作站设置》。

`process_id` 关联 `mes_pro_process` 表，详见 《【生产】工序设置、工艺流程》。
...
3. **IPQC 回调拆行**（`splitPendingAndFinishProduce`）：IPQC 检验完成后，系统根据质量结果将「待检验」行拆分为合格行+不合格行，并生成对应明细。详见 《【质量】过程检验 IPQC》。
4. **执行产出**（`finishProductProduce`）：校验每行的明细数量之和等于行数量后，遍历所有明细创建 IN 库存事务，将产品入库到虚拟线边库。
...
mes\_wm\_product\_produce\_detail 表结构
...
① `work_order_id` 关联 `mes_pro_work_order` 表的 `id` 字段（选填），详见 《【生产】生产工单》。
...
![管理后台 - 产品执行入库](/knowledge-images/6ce92393cf3b.png)
...
【仓库】生产领料、生产退料、物料消耗 【仓库】发货通知、销售出库、销售退货
