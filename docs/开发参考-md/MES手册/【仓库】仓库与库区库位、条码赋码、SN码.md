...
![仓库基础设置模块](/knowledge-images/edca4d07a8b4.svg)
...
![管理后台 - 库位列表](/knowledge-images/e1176335a71c.png)
...
![管理后台 - 条码配置列表](/knowledge-images/b524bc4d21d3.png)

####  条码清单列表

支持按业务类型、业务编码、业务名称、条码内容等条件搜索。列表展示条码格式、业务类型、条码内容、业务编码/名称、状态等信息。

![管理后台 - 条码清单列表](/knowledge-images/f8de73b27771.png)
...
③ `item_id` 关联 `mes_md_item` 表的 `id` 字段（必填），标识该 SN 码对应的产品物料，详见 《【基础】物料产品、分类、计量单位》。
...
![管理后台 - SN码分组列表](/knowledge-images/6333c2329a12.png)
...
![管理后台 - 生成SN码](/knowledge-images/a27edc38027b.png)

####  批量删除

按批次 UUID 删除同批次的所有 SN 码。

【生产】工作记录 【仓库】批次管理、库存现有量、库存事务
