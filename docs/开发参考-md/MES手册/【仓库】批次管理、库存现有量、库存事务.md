...
![库存核心模块](/knowledge-images/ce79252ff22c.svg)
...
① `item_id` 关联 `mes_md_item` 表的 `id` 字段（必填），标识该批次对应的物料，详见 《【基础】物料产品、分类、计量单位》。
...
![管理后台 - 批次追溯列表](/knowledge-images/2dfc51030fe1.png)
...
![管理后台 - 批次详情（正向追溯）](/knowledge-images/5f3f83b7e752.png)

![管理后台 - 批次详情（反向追溯）](/knowledge-images/2cf40e09fe8e.png)
...
④ `warehouse_id`、`location_id`、`area_id` 分别关联仓库、库区、库位表，构成库存的空间维度，详见 《【仓库】仓库与库区库位、条码赋码、SN码》。
...
![管理后台 - 库存现有量列表](/knowledge-images/39a360658eb0.png)
...
【仓库】仓库与库区库位、条码赋码、SN码 【仓库】到货通知、采购入库、采购退货
