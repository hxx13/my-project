...
![客户与供应商模块](/knowledge-images/9717ca1dea3d.svg)
...
![管理后台 - 客户列表](/knowledge-images/2d863535673a.png)

####  新增

点击【新增】按钮，弹出客户新增表单。

![管理后台 - 客户新增](/knowledge-images/f385537e05a3.png)

####  修改

点击【编辑】按钮，弹出客户修改表单，底部包含以下 Tab 页：

![管理后台 - 客户修改](/knowledge-images/f5395e8fc0ee.png)

★ **产品清单**（客户详情 Tab）：展示该客户关联的销售出库明细行，按 `clientId` 过滤关联记录，方便查看该客户购买过哪些产品，包含物料编码、物料名称、规格型号、单位、出库数量、批次号等信息。调用 `WmProductSalesLineApi.getProductSalesLinePage` 接口查询数据（来自 `mes_wm_product_sales_line` 表，后端由 MesWmProductSalesLineController 提供），详见 《【仓库】发货通知、销售出库、销售退货》。

![管理后台 - 客户销售记录](/knowledge-images/4302a42cc720.png)

★ **销售记录**（客户详情 Tab）：展示该客户关联的销售出库单主单列表，按 `clientId` 过滤关联记录，包含出库单编号、出库单名称、销售订单编号、出库日期、单据状态等信息。调用 `WmProductSalesApi.getProductSalesPage` 接口查询数据（来自 `mes_wm_product_sales` 表，后端由 MesWmProductSalesController 提供），详见 《【仓库】发货通知、销售出库、销售退货》。
...
![管理后台 - 供应商列表](/knowledge-images/c9ec580a20f3.png)

####  新增

点击【新增】按钮，弹出供应商新增表单。

![管理后台 - 供应商新增](/knowledge-images/5867dc7a1d8c.png)

####  修改

点击【编辑】按钮，弹出供应商修改表单，底部包含以下 Tab 页：

![管理后台 - 供应商修改](/knowledge-images/60b287e52921.png)

★ **物料清单**（供应商详情 Tab）：展示该供应商关联的采购入库明细行，按 `vendorId` 过滤关联记录，方便查看该供应商供应过哪些物料，包含物料编码、物料名称、规格型号、单位、入库数量、批次号等信息。调用 `WmItemReceiptLineApi.getItemReceiptLinePage` 接口查询数据（来自 `mes_wm_item_receipt_line` 表，后端由 MesWmItemReceiptLineController 提供），详见 《【仓库】到货通知、采购入库、采购退货》。

![管理后台 - 供应商采购记录](/knowledge-images/683999bcc686.png)

★ **采购记录**（供应商详情 Tab）：复用采购入库单行分页接口，按 `vendorId` 过滤后展示入库单编号、采购订单号、物料编码、物料名称、规格型号、单位、入库数量等关联视图。同样调用 `WmItemReceiptLineApi.getItemReceiptLinePage` 接口查询数据（来自 `mes_wm_item_receipt_line` 表，后端由 MesWmItemReceiptLineController 提供），详见 《【仓库】到货通知、采购入库、采购退货》。

【基础】物料产品、分类、计量单位 【基础】车间设置、工作站设置
