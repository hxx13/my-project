...
![工作记录模块](/knowledge-images/2a98328f64c4.svg)
...
② `workstation_id` 关联 `mes_md_workstation` 表的 `id` 字段，记录用户最近一次操作的工作站，详见 《【基础】车间设置、工作站设置》。**下工时不会清空该字段**，仅更新 `type` 和 `clock_out_time`。
...
② `workstation_id` 关联 `mes_md_workstation` 表的 `id` 字段，标识本次操作关联的工作站，详见 《【基础】车间设置、工作站设置》。
...
![管理后台 - 工作记录状态栏](/knowledge-images/ac8826bde707.png)

####  列表

状态栏下方为上下工记录流水列表。支持按用户、工作站、操作类型、操作时间范围等条件搜索。列表展示编号、用户、工作站编码、工作站名称、操作类型（上工/下工）、创建时间等信息。

![管理后台 - 工作记录列表](/knowledge-images/7657d43313e2.png)

【生产】安灯配置、安灯呼叫 【仓库】仓库与库区库位、条码赋码、SN码
