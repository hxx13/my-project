...
![生产工单模块](/knowledge-images/a1e0c7b9cd1a.svg)
...
⑦ `client_id` 关联 `mes_md_client` 表（客户）。前端仅在来源类型为客户订单（`ORDER`）时展示客户选择控件；但后端 `validateWorkOrderSaveData()` 会在产品批次配置 `clientFlag=true` 时强制要求 `clientId`，因此库存备货等其他场景下该字段也可能存在值。详见 《【基础】客户管理、供应商管理》。

⑧ `vendor_id` 关联 `mes_md_vendor` 表（供应商），仅当工单类型为代工或采购时使用，详见 《【基础】客户管理、供应商管理》。
...
![管理后台 - 工单列表](/knowledge-images/947872ebf7f0.png)
...
![管理后台 - 工单新增](/knowledge-images/53a7d3e2f72e.png)

####  修改

点击编码链接查看工单详情（只读），点击【编辑】按钮进入修改表单（可编辑）。详情模式下弹窗底部还提供\*\*「查看条码」\*\*按钮，可打开该工单的条码详情弹窗。弹窗底部包含以下 Tab 页：

![管理后台 - 工单修改](/knowledge-images/4200e77c5b88.png)
...
![管理后台 - 物料需求](/knowledge-images/82a7bb6e26cd.png)
...
![管理后台 - 新增子工单](/knowledge-images/c43eec64bd78.png)

【生产】工序设置、工艺流程 【生产】生产排产、工序流转卡
