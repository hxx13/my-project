...
![生产报工模块](/knowledge-images/e29d5d488eac.svg)
...
⑤ `task_id` 关联 `mes_pro_task` 表的 `id` 字段，标识报工对应的生产任务（要求任务未终态），详见 《【生产】生产排产、工序流转卡》。后端还会校验任务的工单、工作站、工艺路线、工序、产品必须与报工一致。
...
- `mes_wm_item_consume_line`（物料消耗行）：审批通过后若工序配置了 BOM，自动生成消耗记录行。详见 《【仓库】生产领料、生产退料、物料消耗》。
- `mes_wm_product_produce_line`（产品产出行）：关键工序审批通过后自动生成产出记录行。详见 《【仓库】产品产出、产品入库》。
...
- 若未配置 BOM，则跳过物料消耗步骤。详见 《【仓库】生产领料、生产退料、物料消耗》。
...
这两个标识定义在工艺路线工序表（`mes_pro_route_process`）上，详见 《【生产】工序设置、工艺流程》。审批通过后的处理取决于两者的**组合**：
...
![管理后台 - 报工数量区域（非质检工序）](/knowledge-images/175d7311b07c.png)
...
![管理后台 - 报工修改](/knowledge-images/73fd0d851e47.png)
...
![管理后台 - 报工审批](/knowledge-images/2d469a7a5744.png)
...
![管理后台 - BOM物资消耗与产品产出](/knowledge-images/a649dff5d64a.png)
...
![管理后台 - 产品产出](/knowledge-images/3e7828780f06.png)
