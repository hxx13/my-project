...
![设备执行记录模块](/knowledge-images/0ac53442b593.svg)
...
① `plan_id` 关联 `mes_dv_check_plan` 表的 `id` 字段（详见 《【设备】点检保养项目、点检保养方案》）。选填，若指定了方案，创建时系统会根据方案自动生成点检项目明细行。

② `machinery_id` 关联 `mes_dv_machinery` 表的 `id` 字段（详见 《【设备】设备类型、设备台账》），必填。
...
① `plan_id` 关联 `mes_dv_check_plan` 表的 `id` 字段（详见 《【设备】点检保养项目、点检保养方案》），选填。
...
![管理后台 - 保养记录列表](/knowledge-images/db8033ab41b4.png)
...
![管理后台 - 保养记录新增](/knowledge-images/52df89bcc9d7.png)
...
![管理后台 - 保养记录修改](/knowledge-images/102170921fae.png)
...
![管理后台 - 维修单完成维修](/knowledge-images/fe644591a2c6.png)

####  验收

待验收状态下，列表操作列出现【验收】按钮。点击后弹出表单，验收人选择**维修结果**（修复成功 / 报废），调用 `finishRepair`。系统自动写入验收人（`confirm_user_id`）和验收日期（`confirm_date`），状态变为「已确认」。

![管理后台 - 维修单验收](/knowledge-images/8dee5be003e5.png)

【设备】点检保养项目、点检保养方案 【工具】工具类型、工装夹具台账
