...
![出货检验模块](/knowledge-images/60fa9fa30a9c.svg)
...
① `template_id` 关联 `mes_qc_template` 表，**创建时由系统根据 `item_id` + OQC 类型自动匹配**。详见 《【质量】质检方案》。
...
![管理后台 - 出货检验列表](/knowledge-images/96b08bd35d61.png)

####  新增

OQC 检验单有两个创建入口，预填行为不同：

- **从待检任务创建**（推荐）：在 待检任务 页面点击「出货检验」按钮，系统自动预填来源单据信息（来源类型、来源单据编号）以及物料、客户、发货数量、出货日期、检验单名称等字段。其中物料、客户、发货数量为**只读禁用**状态；出货日期和检验单名称虽已预填但仍可修改。检验员还需填写检验单编码、检测数量、合格品数量、不合格品数量、检测人员、检测日期等。
...
![管理后台 - 出货检验新增](/knowledge-images/10e358533bf5.png)
...
![管理后台 - 出货检验修改](/knowledge-images/1bb467b9330b.png)
