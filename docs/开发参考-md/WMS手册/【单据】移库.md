...
① `no` 移库单号，**新增时由前端默认按 `YK + 月日 + 4 位随机数` 生成**（详见 《功能开启》 ①），允许手动修改，由后端校验全局唯一。
...
② `sku_id` 移库 SKU，**新增时由用户从库存选择器选择**（详见 《【库存】库存记录、流水、统计》§1.3），且必须基于**源仓库**的可用库存，不能超过库存余量。
...
![移库单列表](/knowledge-images/40795b700caa.png)

####  新增

通过弹窗 `MovementOrderForm.vue` 完成。表单上半部分是单据基础信息（移库单号 + 自动生成、单据日期、来源仓库、目标仓库、备注），下半部分是明细子表（SKU + 移库数量 + 单价 + 行金额）。

明细的 SKU 通过 `InventorySelect.vue` 库存选择器 选择，弹窗传入**来源仓库**作为 `warehouseId`，只能选源仓库下 `quantity > 0` 的 SKU。

![移库单新增](/knowledge-images/6e1d2b8c713d.png)
...
> 实现细节：service 的 `changeInventory` 方法把每条明细在内存中拆成两个 `WmsInventoryChangeReqDTO.Item` 一并入参，库存事务按统一流程处理（详见 《【库存】库存记录、流水、统计》§3.1）。
...
![移库单网页打印](/knowledge-images/304bf636f61f.png)
