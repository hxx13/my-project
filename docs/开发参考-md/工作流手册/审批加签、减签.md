...
![向前加签](/knowledge-images/a48fbfb61ed1.png)

② 之后，可以看到当前任务变成“待审核”状态，它有加签出来的一个子任务，该子任务的状态是“待审核”状态，如下图所示：

![向前加签子任务](/knowledge-images/554e91574b17.png)
...
![向前加签的实现](/knowledge-images/e7d1b4217f3d.png)
...
![向前加签子任务的实现](/knowledge-images/7f746e75ed12.png)
...
![审批向前加签子任务](/knowledge-images/dab72518e219.png)

② 之后，可以看到当前（原）任务又变回“待审核”状态，加签出来的子任务处于“审核通过”状态，如下图所示：

![审批向前加签子任务后](/knowledge-images/8566c4a46c22.png)
...
![审批向前加签子任务的实现](/knowledge-images/c8d2564d6d31.png)

关键还是 `#handleParentTaskIfSign(...)` 方法，在向前加签的情况下，需要将当前（原）任务又变回“待审核”状态。如下图所示：

图片纠错：最新版本不区分 yudao-module-bpm-api 和 yudao-module-bpm-biz 子模块，代码直接合并到 yudao-module-bpm 模块的 src 目录下，更适合单体项目

![处理加签的父任务](/knowledge-images/78c88cacba74.png)
...
![向后加签](/knowledge-images/bfdeaa0a6dc0.png)

② 之后，可以看到当前任务变成“审核中”状态，它有加签出来的一个子任务，该子任务的状态是“待审核”状态，如下图所示：

![向后加签子任务](/knowledge-images/e97903d34cdd.png)
...
![审批向后加签子任务的实现](/knowledge-images/f2d50b121b62.png)
...
![向后加签子任务的实现](/knowledge-images/eb3d2a854d63.png)
...
![审批当前任务](/knowledge-images/9e28bb998ffe.png)
...
![审批当前任务的实现](/knowledge-images/409a652cd5d4.png)

核心实现在 `#approveAfterSignTask(...)` 方法，如下图所示：

图片纠错：最新版本不区分 yudao-module-bpm-api 和 yudao-module-bpm-biz 子模块，代码直接合并到 yudao-module-bpm 模块的 src 目录下，更适合单体项目

![审批当前任务的实现2](/knowledge-images/503e16f0a2da.png)
...
![审批向后加签子任务](/knowledge-images/8dd81ac287e2.png)
...
![审批向后加签子任务的具体实现](/knowledge-images/f73a5a8ae8e7.png)
...
![减签列表](/knowledge-images/a30317cea94b.png)

② 选择需要减签的任务，点击【减签】按钮，完成减签操作。如下图所示：

![减签结果](/knowledge-images/7cb123233989.png)
...
![获得减签任务列表的实现](/knowledge-images/ba6f73ad67d1.png)

② 执行减签操作，对应 BpmTaskController 的 `#deleteSignTask(...)` 方法。如下图所示：

图片纠错：最新版本不区分 yudao-module-bpm-api 和 yudao-module-bpm-biz 子模块，代码直接合并到 yudao-module-bpm 模块的 src 目录下，更适合单体项目

![减签操作的实现](/knowledge-images/fc1c59a8a439.png)
