...
合同模块，由 `yudao-module-crm` 后端模块的 `contract` 包实现，已经和 [BPM 工作流](/开发参考/支付手册/功能开启.md) 打通审批。
...
④ `contact_last_time` 字段：跟进相关，和线索类似。不重复赘述，详细可见 [《【通用】跟进记录》](/开发参考/CRM手册/【通用】跟进记录、待办事项.md) 文档。

⑤ `owner_user_id` 字段：合同的负责人编号，和线索类似。不重复赘述，详细可见 [《【通用】数据权限》](/开发参考/CRM手册/【通用】数据权限.md) 文档。
