...
具体可见 IotDeviceService 的 `#registerDevice(...)` 方法。详见 [《设备动态注册》](/开发参考/IoT物联网手册/设备动态注册.md)。
...
更多可见 [《设备接入（概述）》](/开发参考/IoT物联网手册/设备接入（概述）.md)。
...
管理产品的物模型，包括属性、服务、事件的定义。详细内容请参考 [《物模型配置》](/开发参考/IoT物联网手册/物模型配置.md) 文档。
