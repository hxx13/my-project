...
① `work_order_id` 关联 `mes_pro_work_order` 表的 `id` 字段（必填），标识该领料单为哪个工单服务。创建时校验工单必须为已确认（或更高）状态，详见 [《【生产】生产工单》](/开发参考/MES手册/【生产】生产工单.md)。

`workstation_id` 关联 `mes_md_workstation` 表，详见 [《【基础】车间设置、工作站设置》](/开发参考/MES手册/【基础】车间设置、工作站设置.md)。

`task_id` 关联 `mes_pro_task` 表（选填），详见 [《【生产】生产排产、工序流转卡》](/开发参考/MES手册/【生产】生产排产、工序流转卡.md)。
...
在「待检验」状态下，点击【执行质检】按钮，系统提示「请前往【质量管理 - 退货检验（RQC）】中进行退料检验操作」。RQC 完成后状态变为「待上架」。详见 [《【质量】退料检验 RQC》](/开发参考/MES手册/【质量】退货检验（RQC）.md)。
...
当前 yudao 实现中，物料消耗没有独立的前端入口页面，主记录由报工审批流程自动生成，属于**系统自动触发**的后台模块。报工审批通过后，系统根据工序级 BOM 用料配置自动生成消耗记录并扣减线边库存。详见 [《【生产】生产报工》](/开发参考/MES手册/【生产】生产报工.md)。
