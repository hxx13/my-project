...
① `template_id` 关联 `mes_qc_template` 表，**创建时由系统自动匹配**（根据 `item_id` + IQC 类型查找适用的质检方案）。详见 [《【质量】质检方案》](/开发参考/MES手册/【质量】质检方案.md)。
...
-   **从待检任务创建**（推荐）：在 [待检任务](/开发参考/MES手册/【质量】待检任务、检验结果、缺陷记录.md) 页面点击「来料检验」按钮，系统自动预填并锁定来源单据（来源类型、来源单据编号）、供应商、物料、本次接收数量，并自动带出检验单名称和来料日期。来源单据区域、供应商、物料、本次接收数量在此场景下均为**禁用不可修改**状态。检验员继续填写供应商批次、合格/不合格数量、检测人员、检测日期、检测结果等。
