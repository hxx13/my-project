...
因此，在 `yudao-module-system` 模块的 `job` 包下，创建 UserSessionTimeoutJob 类，实现 JobHandler 接口，执行用户 Session 超时 Job。如下图所示：

> 📷 *UserSessionTimeoutJob 类*

疑问：为什么添加 @TenantJob 注解？

声明 `@TenantJob` 注解在 Job 类上，实现**并行**遍历每个租户，执行定时任务的逻辑。

更多多租户的内容，可见 [《开发指南 —— SaaS 多租户》](/开发参考/后端手册/SaaS 多租户【字段隔离】.md) 文档。
...
-   Cron 表达式：执行周期，可通过 \[生成表达式\] 功能，进行生成 > 📷 *新建任务*
