...
在 `dict-enum.ts` 文件中，使用 `DICT_TYPE` 枚举了字典的 KEY。如下图所示：
...
-   源码位置：
    -   antd 版本：apps/web-antd/src/components/dict-tag
    -   ele 版本：apps/web-ele/src/components/dict-tag
...
在 `use-dict.ts` 文件中，提供了字典工具类，方法如下：
...
-   实战案例（antd）：apps/web-antd/src/views/system/user/data.ts
-   实战案例（ele）：apps/web-ele/src/views/system/user/data.ts
