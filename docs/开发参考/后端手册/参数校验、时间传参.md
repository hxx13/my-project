...
如果参数校验不通过，会抛出 ConstraintViolationException 异常，被全局的[异常处理](/开发参考/后端手册/异常处理（错误码）.md)捕获，返回“请求参数不正确”的响应。示例如下：
...
-   `views/infra/job/logger/index.vue` 的 `beginTime` 或 `endTime` 参数 > 📷 *单个时间传参*
    
-   `views/system/user/index.vue` 的 `createTime` 参数 > 📷 *多个时间传参*
