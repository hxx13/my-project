...
-   在 [《会签、或签、依次审批》](/开发参考/工作流手册/会签、或签、依次审批.md) 看到的 `${ nrOfCompletedInstances >= nrOfInstances }`、`${ nrOfCompletedInstances == 1 }` 等，就是 UEL 表达式
-   在 [《审批接入（流程表单）》](/开发参考/工作流手册/审批接入（业务表单）.md) 看到的流转条件 `${day > 3}`，也是 UEL 表达式
...
> 📷 *BpmTaskAssignLeaderExpressionConfig*
