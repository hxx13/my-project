...
本小节，讲解如何将前端 + 后端项目，**使用 1Panel**，部署到 dev 开发环境下的一台 Linux 服务器上。如下图所示：
...
② 点击 \[ruoyi-vue-pro\] 所在行的 \[导入备份\] 按钮，再点击 \[从本地上传\] 按钮，执行数据库对应的 `sql` 目录下的 SQL 文件，还要点击 \[恢复\] 按钮，进行初始化。如下图所示：
...
另外，下面有个 `try_files` 配置，是为了解决前端刷新 404 问题，（类似）参考自 《Vue 项目使用宝塔面板部署刷新 404 问题》 文章。
