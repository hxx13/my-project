...
本小节，讲解如何将前端 + 后端项目，**使用 宝塔**，部署到 dev 开发环境下的一台 Linux 服务器上。如下图所示：

> 📷 *Linux 部署*

友情提示：

本文的部署过程，本质和 [Linux 部署](/开发参考/运维手册/Linux 部署.md) 一样，只是使用了宝塔面板，简化了一些操作。

另外，如果你在部署过程中碰到问题，也可以参考 https://articles.zsxq.com/id\_xu72z2zjekbx.html 文章，来自球友的宝塔部署。
