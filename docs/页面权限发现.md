...
本文约定后台 **「页面权限」** 模块如何从代码库自动发现节点，以及与 **HTTP API** 权限的关系。实现代码以 `PagePermissionService` 为准；修改侧栏或路由后请执行 **`node scripts/export-admin-nav-manifest.mjs`**（或前端 `npm run build`），并在 **页面权限设置** 中点击 **「重新扫描」** 同步数据库。

## 0. 权威清单（manifest）

| 产出 | 路径 | 说明 |
|------|------|------|
| 构建脚本 | `scripts/export-admin-nav-manifest.mjs` | 读取 `adminNavRegistry.ts` + `router/index.tsx` |
| 运行时清单 | `src/main/resources/page-permission/admin-nav.manifest.json` | 后端扫描主数据源（classpath） |

manifest 每条记录含 `path`、`label`、`fallbackMinRole`、`groupTitle`；扫描时 **优先使用 manifest 的 fallbackMinRole**，再退回 `inferWebMinRole` 前缀推断。另会合并 `admin_nav_config` 表中 ITEM 路径作为侧栏 ENTRY 补充。
...
1. 合并包含 `adminNavRegistry.ts` 或 router 改动的分支后：运行 `node scripts/export-admin-nav-manifest.mjs`，再在 **页面权限设置** 执行 **重新扫描**。
