...
import { fetchSpaceTree, type SpaceNode } from "@/api/domains/inventory.api";
import { Portal } from "@/components/Portal";
import { useMultiSelectPopover } from "@/features/admin/violations/shared/useMultiSelectPopover";
import { cn } from "@/lib/utils";

/**
 * 通用「空间选择器」——物品台账 / 物资转移 / 移动空间 的统一入口。
 *
 * 与资产侧 `AssetLocationTreeSelect` 同一套交互与取值口径，只是数据源换成空间树：
 * 树状分级、**默认全部收起**、带搜索；`value` 是**全路径文本**（如「全楼 / 1F / A101」），
 * `onChange` 同时回传节点 id，转移类调用方直接拿 id 调物品转移接口。
 * 浮层经 Portal 以 fixed 定位挂到 body（复用 useMultiSelectPopover），弹窗/滚动容器内都不会被裁切。
 */

/** 排除指定节点及其整棵子树（移动空间时不能把节点挪进自己的子树） */
function excludeSubtrees(nodes: SpaceNode[], exclude?: Set<number>): SpaceNode[] {
  if (!exclude || exclude.size === 0) return nodes;
  return nodes.filter((n) => !exclude.has(n.id)).map((n) => ({ ...n, children: excludeSubtrees(n.children ?? [], exclude) }));
}

/** 搜索过滤：命中的节点保留整棵子树，只有子孙命中的节点保留「命中链」 */
function filterKeepSubtrees(nodes: SpaceNode[], keyword: string): SpaceNode[] {
...
export type SpaceTreeSelectProps = {
  /** 空间全路径文本；空串表示未选 */
  value: string;
  ...
  disabled?: boolean;
  id?: string;
  /** 这些节点及其子孙不出现（如「移动空间」时排除自己与自己的子树） */
  excludeIds?: Set<number>;
};

export function SpaceTreeSelect({
  value,
  onChange,
  clearable = false,
  placeholder = "选择空间",
  ...
  id,
  excludeIds,
}: SpaceTreeSelectProps) {
  // 与左树共用同一个 query key，缓存共享、不额外发请求
  const { data: tree = [] } = useQuery({ queryKey: ["inventory", "spaces"], queryFn: fetchSpaceTree });
  ...
  const revealChild = (childId?: number) => {
    ...
    requestAnimationFrame(() => {
      panelRef.current?.querySelector(`[data-space-id="${childId}"]`)?.scrollIntoView({ block: "nearest" });
    });
  };

  const renderNode = (node: SpaceNode, depth: number, prefix: string) => {
      ...
      {open && (
        <Portal>
          ...
          >
            ...
            {visible.length === 0 ? (
              <div className="px-2 py-3 text-center text-[11px] text-[var(--app-color-text-tertiary)]">没有匹配的空间</div>
            ) : (
...
export default SpaceTreeSelect;
