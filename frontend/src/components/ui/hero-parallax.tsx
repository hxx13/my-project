...
export const HeroParallax = ({
  ...
  // REVERSED: images fly DOWN (0 → 500) into background, original: fall from above (-700 → 500)
  const translateY = useSpring(
    useTransform(scrollYProgress, [0, 0.3], reversed ? [0, 500] : [-700, 500]),
    springConfig
  );

  if (reversed) {
    return (
      <div ref={ref} className="h-[300vh] py-20 overflow-hidden antialiased relative flex flex-col [perspective:1000px] [transform-style:preserve-3d]">
        {/* Images FIRST — they fly UP to reveal header below */}
        <motion.div style={{ rotateX, rotateZ, translateY, opacity }} className="z-20">
        ...
        {/* Header BELOW — revealed as images fly UP */}
    ...
    >
      <a href={product.link} className="block group-hover/product:shadow-2xl">
        <img src={product.thumbnail} height="600" width="600"
          className="object-cover object-left-top absolute h-full w-full inset-0" alt={product.title} />
      </a>
