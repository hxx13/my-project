...
import {
  createAnalyticsReportShare,
  fetchAnalyticsReportShare,
...
type Props = {
  ...
  viewCount: number;
  onClose: () => void;
  onImported?: (views: AnalyticsUserView[]) => void;
};
...
export function AnalyticsViewShareModal({
  ...
  const loadActiveShare = useCallback(async () => {
    ...
    try {
      const s = await fetchAnalyticsReportShare(reportKey);
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      const res = await importAnalyticsViewShare(trimmed, nameSuffix.trim() || undefined);
      // 保存后仅合并导入的配置行，禁止整表 load — post-save-no-full-refresh.mdc
      onImported?.(res.views);
      toast.success(
        `已导入 ${res.viewCount} 条配置：${res.importedAuditLogs} 条清算、${res.importedInsights} 条 AI 解读`
      );
      onClose();
    } catch (e) {
          ...
          {mode === "create" ? (
            <>
              <p className="text-xs text-neutral-500">
                将封箱当前账号下全部 {viewCount} 条统计配置，及其清算快照与 AI 解读；他人导入后为独立副本。重新生成后旧码作废。
              </p>
              {loadingShare ? (
                ...
                <div>
                  ...
                  <p className="mt-2 text-xs text-neutral-500">
                    {shareMeta.viewCount} 条配置 · {shareMeta.auditLogCount} 条清算 · {shareMeta.insightCount} 条 AI
                    解读 · 有效期至 {shareMeta.expiresAt.slice(0, 10)} · 剩余导入 {shareMeta.importsRemaining}/
                    {shareMeta.maxImports} 次
                  </p>
                  {shareMeta.viewNames.length > 0 ? (
                    <p className="mt-1 text-[11px] text-neutral-400">
                      含：{shareMeta.viewNames.slice(0, 5).join("、")}
                      {shareMeta.viewNames.length > 5 ? ` 等 ${shareMeta.viewNames.length} 条` : ""}
                    </p>
                  ) : null}
                </div>
              ) : (
              ...
              <div className="rounded-lg border border-neutral-100 bg-neutral-50/80 p-3">
                <p className="mb-2 text-xs font-medium text-neutral-700">
                  {shareMeta ? "重新生成（按当前全部配置重新封箱）" : "生成分享码"}
                </p>
                ...
                <div className="mt-3 rounded-lg border border-neutral-200 bg-neutral-50 p-3 text-xs text-neutral-700">
                  <p>
                    <span className="font-medium">
                      {preview.viewCount != null && preview.viewCount > 1
                        ? `${preview.viewCount} 条配置`
                        : preview.viewName}
                    </span>{" "}
                    · 来自 {preview.ownerDisplayName}
                  </p>
                  {preview.viewNames && preview.viewNames.length > 0 ? (
                    <p className="mt-1 text-neutral-500">
                      {preview.viewNames.slice(0, 6).join("、")}
                      {preview.viewNames.length > 6 ? ` 等 ${preview.viewNames.length} 条` : ""}
                    </p>
                  ) : null}
                  <p className="mt-1">
              ...
              {preview ? (
                <>
                  <label className="mt-3 block text-xs text-neutral-600">
                    导入后名称后缀（可选，将追加到每条配置名后）
                    <input
                      value={nameSuffix}
                      onChange={(e) => setNameSuffix(e.target.value)}
                      placeholder="例如： (来自 张三)"
                  ...
                  >
                    {importing ? "导入中…" : `确认导入${preview.viewCount != null ? `（${preview.viewCount} 条）` : ""}`}
                  </button>
                </>
              ) : null}
            </>
