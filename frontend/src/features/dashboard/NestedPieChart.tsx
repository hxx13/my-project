...
export function NestedPieChart() {
    ...
    const pdNightColors = nightPalette
        ? [
              nightPalette.exit,
              hexToRgba(nightPalette.exit, 0.85),
              hexToRgba(nightPalette.exit, 0.7),
              hexToRgba(nightPalette.exit, 0.55),
              hexToRgba(nightPalette.exit, 0.4),
          ]
        : [];
    const pxNightColors = nightPalette
        ? [
              nightPalette.entry,
              hexToRgba(nightPalette.entry, 0.85),
              hexToRgba(nightPalette.entry, 0.7),
              hexToRgba(nightPalette.entry, 0.55),
              hexToRgba(nightPalette.entry, 0.4),
          ]
        : [];
    ...
    const option = useMemo(() => {
        const emptyPuxi = [
            {
                ...
                value: 1,
                itemStyle: {
                    color: sciFi ? "rgba(192,132,252,0.12)" : night ? hexToRgba(nightPalette!.entry, 0.1) : "rgba(142,68,173,0.08)",
                    borderColor: sciFi ? "rgba(192,132,252,0.45)" : night ? hexToRgba(nightPalette!.entry, 0.35) : "rgba(142,68,173,0.3)",
                    borderWidth: 1,
        ...
        const emptyPudong = [
            {
                ...
                value: 1,
                itemStyle: {
                    color: sciFi ? "rgba(56,189,248,0.12)" : night ? hexToRgba(nightPalette!.exit, 0.12) : "rgba(45,92,247,0.08)",
                    borderColor: sciFi ? "rgba(56,189,248,0.45)" : night ? hexToRgba(nightPalette!.exit, 0.35) : "rgba(45,92,247,0.3)",
                    borderWidth: 1,
                    borderType: "dashed" as const,
                },
                tooltip: { show: false },
        ...
        const sliceBorder = sciFi ? "rgba(2, 6, 23, 0.92)" : night ? hexToRgba(nightPalette!.surfaceProfile, 0.65) : "#ffffff";
        const labelColor = sciFi ? "#cbd5e1" : night ? nightPalette!.accentInk : "#475569";
        const labelLineColor = sciFi ? "rgba(56, 189, 248, 0.45)" : night ? hexToRgba(nightPalette!.accentWarm, 0.28) : "#cbd5e1";
        const pxColors = sciFi ? PX_COLORS_SF : night ? pxNightColors : PX_COLORS;
        const pdColors = sciFi ? PD_COLORS_SF : night ? pdNightColors : PD_COLORS;
        const shadowBlur = sciFi ? 18 : night ? 0 : 10;
        const shadowBlurOuter = sciFi ? 22 : night ? 0 : 15;

        return {
            tooltip: {
                trigger: "item" as const,
                backgroundColor: sciFi ? "rgba(2, 6, 23, 0.92)" : night ? hexToRgba(nightPalette!.surfaceStudent, 0.92) : "rgba(255,255,255,0.9)",
                extraCssText: sciFi
                ...
                borderRadius: 12,
                borderColor: sciFi ? "rgba(56, 189, 248, 0.35)" : night ? hexToRgba(nightPalette!.accentWarm, 0.28) : "rgba(0,0,0,0.05)",
                textStyle: { color: sciFi ? "#e2e8f0" : night ? nightPalette!.accentInk : "#0f172a" },
                formatter: "{b}: {c}次 ({d}%)",
            },
            series: [
                {
                ...
                },
                {
                    ...
                    itemStyle: {
                        ...
                        borderColor: sliceBorder,
                        borderWidth: 2,
                        shadowBlur: shadowBlurOuter,
                        shadowOffsetX: sciFi ? 2 : 0,
                        shadowOffsetY: sciFi ? 2 : 0,
    ...
    }, [sciFi, night, nightPalette, pdNightColors, pxNightColors, hasPudongData, hasPuxiData, pieStats]);
    ...
    const legendShell = sciFi
        ...
        : night
          ? `${DASH_NIGHT_CLASS.legend} px-2 py-1 backdrop-blur-md z-10`
          : "bg-white/70 px-2 py-1 rounded-[12px] border border-white/80 backdrop-blur-md z-10 shadow-[0_4px_10px_rgba(0,0,0,0.05)]";

    return (
        <div className={`w-full h-full relative ${night ? DASH_NIGHT_CLASS.chartArea : ""}`}>
            <div className={`absolute top-0 right-0 flex items-center gap-2 ${legendShell}`}>
                <div
                    className={`flex items-center gap-1 font-bold text-[11px] ${
                        sciFi ? "text-cyan-300" : night ? DASH_NIGHT_CLASS.legendSteel : "text-[#2d5cf7]"
                    }`}
                >
                ...
                <div
                    className={`w-[1px] h-3 ${sciFi ? "bg-cyan-500/30" : night ? "bg-[var(--dash-night-border-warm)]" : "bg-black/10"}`}
                />
                <div
                    className={`flex items-center gap-1 font-bold text-[11px] ${
                        sciFi ? "text-fuchsia-300" : night ? DASH_NIGHT_CLASS.legendWarm : "text-[#8e44ad]"
                    }`}
                >
            ...
            <ReactECharts key={sciFi ? "sf" : night ? "night" : "std"} option={option} style={{ height: "100%", width: "100%" }} />
        </div>
    );
}
