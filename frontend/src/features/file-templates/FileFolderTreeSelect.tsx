/**
 * FileFolderTreeSelect — 文件模板库 · 文件夹选择器。
 *
 * 本文件是 `components/admin/AssetLocationTreeSelect.tsx` 的复制品，只换了数据源与节点类型；
 * 两处都要改时需同步（这是有意为之的重复——泛型化会波及资产记录页，风险大于收益）。
 */

import { useCallback, useMemo, useRef, useState } from "react";
import { ChevronDown, ChevronRight, X } from "lucide-react";
import type { FileFolderNode } from "@/api/domains/fileFolders.api";
import { useFileFolderTree } from "@/api/hooks/useFileFolders";
import { Portal } from "@/components/Portal";
import { useMultiSelectPopover } from "@/features/admin/violations/shared/useMultiSelectPopover";
import { cn } from "@/lib/utils";

/**
 * 通用「文件夹选择器」——文件模板归属文件夹字段的统一入口。
 *
 * 交互：树状分级，**默认全部收起**；带搜索，命中项保留整棵子树可继续下钻。
 * 取值：`value` 是文件夹**全路径文本**；`onChange` 同时回传节点 id。
 *
 * 浮层经 Portal 以 fixed 定位挂到 body（复用 useMultiSelectPopover），弹窗/表格/滚动容器内都不会被裁切。
 */

/** 搜索过滤：命中的节点保留整棵子树，只有子孙命中的节点保留「命中链」 */
function filterKeepSubtrees(nodes: FileFolderNode[], keyword: string): FileFolderNode[] {
  const k = keyword.trim().toLowerCase();
  if (!k) return nodes;
  const out: FileFolderNode[] = [];
...
/** 排除指定节点及其整棵子树（移动文件夹时不能把节点挪进自己的子树） */
function excludeSubtrees(nodes: FileFolderNode[], exclude?: Set<number>): FileFolderNode[] {
...
export type FileFolderTreeSelectProps = {
  /** 文件夹全路径文本；空串表示未选 */
  value: string;
  ...
  disabled?: boolean;
  id?: string;
  /** 这些节点及其子孙不出现（如「移动文件夹」时排除自己与自己的子树） */
  excludeIds?: Set<number>;
};

export function FileFolderTreeSelect({
  value,
  onChange,
  clearable = false,
  placeholder = "选择文件夹",
  ...
  id,
  excludeIds,
}: FileFolderTreeSelectProps) {
  const { data: tree = [] } = useFileFolderTree();
  ...
  const renderNode = (node: FileFolderNode, depth: number, prefix: string) => {
      ...
      {open && (
        <Portal>
          ...
          >
            ...
            {visible.length === 0 ? (
              <div className="px-2 py-3 text-center text-[11px] text-[var(--app-color-text-tertiary)]">没有匹配的文件夹</div>
            ) : (
...
export default FileFolderTreeSelect;
