...
export default function ContactPageEditor() {
  ...
  useEffect(() => {
    ...
    if (Array.isArray(ext.contacts) && ext.contacts.length > 0) {
      setContacts(ext.contacts as ContactItem[]);
    } else {
      // fallback: 从 summary | 分割
      const labels = ["地址", "电话", "邮箱", "办公时间"];
      const icons = ["MapPin", "Phone", "Mail", "Clock"];
      const parts = (selected.summary || "").split("|").map((s) => s.trim()).filter(Boolean);
      if (parts.length > 0) {
        setContacts(parts.map((v, i) => ({ label: labels[i] || `项目${i + 1}`, icon: icons[i] || "MapPin", value: v })));
      } else {
        setContacts([
          { label: "地址", icon: "MapPin", value: "" },
          { label: "电话", icon: "Phone", value: "" },
          { label: "邮箱", icon: "Mail", value: "" },
          { label: "办公时间", icon: "Clock", value: "" },
        ]);
      }
    }
  ...
  const publish = useCallback(() => {
    ...
    updateMut.mutate({
      id: selected.id,
      body: { title, summary: newSummary, extensionJson: ext, status: "PUBLISHED" },
    }, {
      ...
      <div style={{ maxWidth: 1060, margin: "0 auto", display: "flex", gap: 20 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {!selected ? (
            ...
            <>
              <div style={card}>
                <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 14 }}>📞 联系我们 — 基础信息</h3>
              ...
              <div style={card}>
                <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 14 }}>📋 联系方式条目（{contacts.length} 条）</h3>
                {contacts.map((item, i) => (
                  <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
                    <select style={{ ...selectStyle, flex: 1, maxWidth: 110 }} value={item.icon} onChange={(e) => { const n = [...contacts]; n[i] = { ...n[i], icon: e.target.value }; setContacts(n); }}>
                      {ICON_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                    </select>
                    <input style={{ ...inputStyle, flex: 1, maxWidth: 100 }} placeholder="标签" value={item.label} onChange={(e) => { const n = [...contacts]; n[i] = { ...n[i], label: e.target.value }; setContacts(n); }} />
                    <input style={{ ...inputStyle, flex: 2 }} placeholder="内容" value={item.value} onChange={(e) => { const n = [...contacts]; n[i] = { ...n[i], value: e.target.value }; setContacts(n); }} />
                    {contacts.length > 1 && (
                      <button style={{ padding: "4px 10px", borderRadius: 6, border: "1px solid #d4c9b8", background: "white", fontSize: 11, cursor: "pointer", color: "#999" }} onClick={() => setContacts(contacts.filter((_, j) => j !== i))}>✕</button>
                    )}
                  </div>
                ))}
