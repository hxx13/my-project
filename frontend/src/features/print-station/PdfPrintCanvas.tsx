  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 按 600 DPI 渲染 —— 这台机器的打印引擎就是 600×600 dpi。
        //
        // 为什么要喂到跟引擎同分辨率：喂 300 DPI 的话，驱动还得**放大一倍**再送进引擎，
        // 那一步是插值，细节在那时候就丢了。1:1 喂进去才是这个引擎能打出的上限。
        // （原先固定 2 倍只有 144 DPI，A4 上的小字明显发虚 —— 打印出来才发现的。
        //   屏幕上预览看着清楚毫无参考价值：那是缩放显示，把糊掉的部分掩盖了。）
        const TARGET_DPI = 600;
        /** 单页像素上限：防 A3/图纸这类大页面把内存吃爆（A4@600dpi ≈ 35 MP） */
