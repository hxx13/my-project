  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 按 300 DPI 渲染 —— 打印标准（72pt = 1 英寸，scale = 300/72 ≈ 4.17）。
        //
        // 试过 600 DPI（跟这台机引擎 1:1），纸面上肉眼几乎看不出差别，
        // 但单页 spool 从 25 MB 涨到 99.5 MB、渲染慢 4 倍 —— 不划算，退回 300。
        // 底线是 144 DPI（原先是固定 2 倍），那个是真的糊。
        //
        // 注意屏幕上预览看着清不清楚**毫无参考价值**：那是缩放显示，
        // 把糊掉的部分掩盖了，只有纸面按真实尺寸落墨才看得出来。
        const TARGET_DPI = 300;
        /** 单页像素上限：防 A3/图纸这类大页面把内存吃爆 */
