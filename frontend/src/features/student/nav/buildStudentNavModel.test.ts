...
describe("buildStudentNavModel fallback", () => {
  it("falls back to registry (7 groups / 11 items) when config is empty", async () => {
    const result = await buildStudentNavModel({ role: "MEMBER", permNodes: [] });

    expect(result.sidebarGroups).toHaveLength(7);
    expect(result.flatNavigableItems).toHaveLength(11);
  });
  ...
  it("excludes a hidden registry item from the fallback output", async () => {
    ...
    expect(result.sidebarGroups).toHaveLength(7);
    expect(result.flatNavigableItems).toHaveLength(10);
