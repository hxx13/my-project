...
import OrderAnalyticsReportPanel from "@/features/analytics/components/OrderAnalyticsReportPanel";
...
const ORDER_ANALYTICS_KEY = "order_analytics";

const ANALYTICS_REPORT_KEYS = [ISOLATION_REPORT_KEY, CAGE_REPORT_KEY, STUDENT_ACTIVITY_KEY, CAGE_SPECIAL_STATUS_KEY, ORDER_ANALYTICS_KEY] as const;
...
export default function AdminAnalyticsPage() {
  ...
  const isAnalyticsReport = ANALYTICS_REPORT_KEYS.includes(activeKey as (typeof ANALYTICS_REPORT_KEYS)[number]) && activeKey !== ORDER_ANALYTICS_KEY;
  ...
  return (
    <AdminFullWidthPage>
      ...
      <div className="flex flex-col gap-6 lg:flex-row px-4">
        ...
        <div className={cn("min-w-0 flex-1 min-h-[200px]", activeKey !== ORDER_ANALYTICS_KEY && "max-h-[calc(100dvh-var(--admin-chrome-offset))] overflow-y-auto")} style={activeKey === ORDER_ANALYTICS_KEY ? { height: "calc(100dvh - var(--admin-chrome-offset))" } : undefined}>
          {active && activeKey !== ORDER_ANALYTICS_KEY ? (
          ...
          {activeKey === ORDER_ANALYTICS_KEY ? <OrderAnalyticsReportPanel /> : null}
          {isMaterialStats ? <MaterialStatsReportPanel /> : null}
          {!isAnalyticsReport && !isMaterialStats && activeKey !== ORDER_ANALYTICS_KEY ? (
  ...
  return (
    <div className={cn("shrink-0 relative group", !report.available && "cursor-not-allowed opacity-50")}>
      <button
        ...
        onClick={onSelect}
        className={cn(
          "w-full rounded-xl border px-3 py-2.5 text-left text-sm transition lg:w-full flex items-center gap-2",
          active
      ...
      >
        ...
        {!report.available ? <span className="text-xs opacity-70">（筹备中）</span> : null}
        <span
          onClick={(e) => { e.stopPropagation(); onToggleFav(); }}
          className={cn(
            "shrink-0 rounded transition",
            isFavorite ? "text-[#FFD700]" : cn("text-[var(--app-color-text-tertiary)] hover:text-[#FFD700]", !active && "opacity-0 group-hover:opacity-100"),
          )}
          title={isFavorite ? "取消收藏" : "收藏此报表"}
        >
          <Star className={cn("h-4 w-4", isFavorite ? "fill-[#FFD700] text-[#FFD700]" : "")} />
        </span>
      </button>
    </div>
  );
