...
export default function AdminPersonnelPage() {
  ...
  const [wxEditDraft, setWxEditDraft] = useState("");
  const [wxEditSaving, setWxEditSaving] = useState(false);
  ...
  return (
    <AdminPageShell>
                ...
                <tr key={row.id} className="border-t border-[var(--twin-hairline)]">
                  ...
                  <td className="px-2 py-1.5 align-middle">
                    {row.wxPusherUid ? (
                      <button
                        type="button"
                        className="inline-flex items-center gap-1 rounded-full border border-indigo-200 bg-indigo-50 px-2 py-0.5 text-[11px] font-medium text-indigo-800 hover:bg-indigo-100"
                        onClick={() => {
                          setWxEditOpen(row.id);
                          setWxEditDraft(row.wxPusherUid ?? "");
                        }}
                      >
                        已绑定
                      </button>
      ...
      {wxEditOpen ? <Portal><div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog">
          <div className="w-full max-w-sm rounded-twin-xl border border-[var(--twin-hairline)] bg-[var(--twin-canvas)] p-4 shadow-twin-level-4">
            <div className="text-sm font-semibold text-[var(--twin-ink)]">设置 WxPusher 推送 UID</div>
            <p className="mt-1 text-xs text-[var(--twin-mute)]">
              为 <strong>{(() => { const r = personnelRows.find((p) => p.id === wxEditOpen); return r ? (r.name || r.username || wxEditOpen) : wxEditOpen; })()}</strong> 设置 WxPusher 用户 UID
            </p>
            <p className="mt-1 text-[11px] text-[var(--twin-mute)]">
              关注微信公众号「WxPusher新消息服务」→ 我的 → 我的UID → 复制到此处
            </p>
            <input
              type="text"
              value={wxEditDraft}
              onChange={(e) => setWxEditDraft(e.target.value)}
              maxLength={256}
              className="mt-3 w-full rounded-twin-sm border border-[var(--twin-hairline)] px-3 py-2 text-sm text-[var(--twin-ink)] bg-[var(--twin-canvas)]"
              placeholder="请输入 WxPusher UID"
              onKeyDown={(e) => {
                if (e.key === "Enter" && wxEditDraft.trim()) {
                  e.preventDefault();
                  const btn = document.getElementById("personnel-wx-submit-btn") as HTMLButtonElement | null;
                  btn?.click();
                }
              }}
            />
            <div className="mt-4 flex justify-end gap-2">
              <button
                type="button"
                className="rounded-md border border-[var(--twin-hairline)] px-3 py-1.5 text-xs text-[var(--twin-mute)] hover:bg-[var(--twin-canvas-soft)]"
                onClick={() => { setWxEditOpen(null); setWxEditDraft(""); }}
              >
                取消
              </button>
              <button
                id="personnel-wx-submit-btn"
                type="button"
                className="rounded-md bg-indigo-600 px-3 py-1.5 text-xs text-white hover:bg-indigo-700 disabled:opacity-50"
                disabled={wxEditSaving || !wxEditOpen || !wxEditDraft.trim()}
                onClick={async () => {
                  if (!wxEditOpen || !wxEditDraft.trim()) return;
                  setWxEditSaving(true);
                  try {
                    const token = authStorage.getToken();
                    const res = await fetch(`/api/admin/personnel/${encodeURIComponent(wxEditOpen)}/wx-pusher-uid`, {
                      method: "PUT",
                      headers: {
                        "Content-Type": "application/json",
                        Authorization: "Bearer " + token,
                      },
                      body: JSON.stringify({ wxPusherUid: wxEditDraft.trim() }),
                    });
                    if (!res.ok) {
                      const errData = await res.json().catch(() => ({}));
