...
export default function AdminSuppliesAuditExportPage() {
  ...
  const maxAuditPages = Math.max(
    Math.ceil(auditTotal / auditSize),
    Math.ceil(restoredTotal / auditSize),
    1
  );
    ...
    <AdminPageShell>
      <div className="flex flex-col max-h-[calc(100dvh-var(--admin-chrome-offset))] min-h-[200px]">
        <AdminFormCard className="shrink-0 mb-3">
          {/* First row: pageLabel + tab buttons */}
          ...
          {/* Second row: tab-specific filter controls */}
          {tab === "personal" ? (
            ...
            <div className="flex flex-wrap items-end gap-3">
              ...
              <span className="pb-2 text-xs text-[var(--twin-mute)]">※ 下拉中优先显示有流水或领用还原的物品</span>
            </div>
          )}
        </AdminFormCard>
        ...
        <div className="flex-1 min-h-0 flex flex-col rounded-xl border border-[var(--app-color-border-default)] bg-[var(--app-color-surface-container)] shadow-sm overflow-hidden">
          <div className="flex-1 min-h-0 overflow-auto">
            {tab === "personal" ? (
              <>
                {/* Personal tab: note text */}
                {personalMode === "multi" ? (
                  <p className="p-3 text-xs text-[var(--twin-mute)]">
                ...
                {personalMode === "single" ? (
                  loadingPersonal ? (
                    ...
                    <div>
                      <dl className="m-3 grid gap-2 rounded-twin-lg border border-[var(--twin-hairline)] bg-[var(--twin-canvas-soft)] p-3 text-xs text-[var(--twin-body)] sm:grid-cols-2">
                        <div>
                        ...
                        <table className="w-full min-w-max text-left text-sm whitespace-nowrap border-collapse">
                          <thead className="border-b-2 border-[var(--app-color-border-strong)]">
                            <tr className="sticky top-0 z-[2] bg-[var(--app-color-surface-hover)] text-[var(--app-color-text-secondary)] font-bold shadow-[var(--app-elevation-card)]">
                              ...
                              <th className="p-3">领用申请时间</th>
                              <th className="p-3">出库完成时间</th>
                              <th className="p-3">申请领用数量</th>
                              <th className="p-3">出库数量</th>
                              <th className="p-3">出入库类型</th>
                              <th className="p-3">出库处理人</th>
                              <th className="p-3">申请领用人</th>
                          ...
                          <tbody>
                            {(claimDetail.lines || []).map((line) => (
                              <tr key={line.id} className="hover:bg-[var(--twin-canvas-soft)]">
                                <td className="p-3">{line.snapshotName}</td>
                                <td className="p-3">{formatSpecLabel(line.specSnapshot) || "—"}</td>
                                <td className="p-3">{toTimeText(claimDetail.createdAt)}</td>
                                <td className="p-3">{toTimeText(claimDetail.fulfilledAt)}</td>
                                <td className="p-3">{line.qty}</td>
                                <td className="p-3">{line.fulfilledQty ?? 0}</td>
                                <td className="p-3">{lineIoType(claimDetail.status, line.fulfilledQty ?? 0)}</td>
                                <td className="p-3">{claimDetail.fulfilledByName || claimDetail.fulfilledBy || "-"}</td>
                                <td className="p-3">{claimDetail.applicantName || claimDetail.userId || "-"}</td>
                                <td className="p-3 text-[var(--twin-mute)]">—</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                    ...
                    <div className="flex min-h-[160px] items-center justify-center text-sm text-[var(--twin-mute)]">未找到该领用单详情。</div>
                  ) : (
                    <div className="flex min-h-[160px] items-center justify-center text-sm text-[var(--twin-mute)]">请选择领用单查看明细。</div>
                  )
                ) : null}
                ...
                {personalMode === "multi" ? (
                  rangePack ? (
                    <>
                      <p className="px-3 py-2 text-xs text-[var(--twin-body)]">
                      ...
                      {aggregateFlatRows.length > 0 ? (
                            ...
                            <tbody>
                              {aggregateFlatRows.map((row) => (
                                <tr key={row.rowKey} className="hover:bg-[var(--twin-canvas-soft)]">
                                  <td className="p-3">{row.snapshotName}</td>
                                  <td className="p-3">{formatSpecLabel(row.specSnapshot) || "—"}</td>
                                  <td className="p-3">{toTimeText(row.createdAt)}</td>
                                  <td className="p-3">{toTimeText(row.fulfilledAt)}</td>
                                  <td className="p-3">{row.qty}</td>
                                  <td className="p-3">{row.fulfilledQty}</td>
                                  <td className="p-3">{row.ioType}</td>
                                  <td className="p-3">{row.fulfilledByDisp}</td>
                                  <td className="p-3">{row.applicantDisp}</td>
                                  <td className="p-3 text-[var(--twin-mute)]">—</td>
                                </tr>
                              ))}
                            </tbody>
                        ...
                        <div className="flex min-h-[160px] items-center justify-center text-sm text-[var(--twin-mute)]">该区间内无明细行可展示。</div>
                      ) : null}
                    </>
                  ) : loadingRange ? (
                    <div className="flex min-h-[200px] items-center justify-center text-sm text-[var(--twin-mute)]">汇总中…</div>
                  ) : (
                    <div className="flex min-h-[160px] items-center justify-center text-sm text-[var(--twin-mute)]">请填写日期区间以查看汇总数据。</div>
                  )
                ) : null}
              </>
            ) : (
              <>
                ...
                <div>
                  <table className="w-full min-w-max text-left text-sm whitespace-nowrap border-collapse">
                    <thead className="border-b-2 border-[var(--app-color-border-strong)]">
                      <tr className="sticky top-0 z-[2] bg-[var(--app-color-surface-hover)] text-[var(--app-color-text-secondary)] font-bold shadow-[var(--app-elevation-card)]">
                        ...
                        <th className="p-3">现存</th>
                        <th className="p-3">处理人</th>
                        <th className="p-3">领用人</th>
                        <th className="p-3">备注</th>
                      </tr>
                    </thead>
                    <tbody>
                      {mergedAuditRows.map((row) => {
                        ...
                        if (row.kind === "movement") {
                          ...
                          return (
                            <tr key={rk} className="hover:bg-[var(--twin-canvas-soft)]">
                              <td className="p-3">{toTimeText(m.createdAt)}</td>
                              <td className="p-3">{m.itemName || "-"}</td>
                              <td className="p-3 text-[var(--twin-mute)]">{formatSpecLabel((m as { specSnapshot?: string }).specSnapshot) || "—"}</td>
                              <td className="p-3">{movementInOutLabelRow(m.movementType, q)}</td>
                              <td className="p-3">{movementChangeQtySignedDisplay(m)}</td>
                              <td className="p-3">
                                {auditTableEditing ? (
                                  <input
                              ...
                              <td className="p-3">{m.operatorName || m.operatorUserId || "-"}</td>
                              <td className="p-3">{m.applicantName || m.applicantUserId || "-"}</td>
                              <td className="p-3">
                                {auditTableEditing ? (
                                  <input
                        ...
                        return (
                          <tr key={rk} className="hover:bg-violet-50/40">
                            <td className="p-3">{toTimeText(h.outboundTime)}</td>
                            <td className="p-3">{h.itemName || "-"}</td>
                            <td className="p-3 text-[var(--twin-body)]">{formatSpecLabel((h as { specSnapshot?: string }).specSnapshot) || "—"}</td>
                            <td className="p-3">出库</td>
                            <td className="p-3">{changeSigned}</td>
                            <td className="p-3">
                              {auditTableEditing ? (
                                <input
                                  className="w-20 min-w-0 rounded-twin-sm border border-[var(--twin-hairline-strong)] px-1 py-0.5"
                            ...
                            <td className="p-3">{h.fulfilledByName || h.fulfilledByUserId || "-"}</td>
                            <td className="p-3">{h.applicantName || h.applicantUserId || "-"}</td>
                            <td className="p-3">
                              {auditTableEditing ? (
                                <input
                                  className="min-w-[8rem] max-w-[14rem] rounded-twin-sm border border-[var(--twin-hairline-strong)] px-1 py-0.5"
                  ...
                  {mergedAuditRows.length === 0 && !auditFetching ? (
                    <div className="flex min-h-[200px] items-center justify-center text-sm text-[var(--twin-mute)]">
                      {selectedItemId === "" ? "请选择物品后查看审计数据。" : "本页暂无数据（流水与领用还原均为空，或翻页后无记录）"}
                    </div>
                  ) : null}
                  {auditFetching ? (
                    <div className="flex min-h-[200px] items-center justify-center text-sm text-[var(--twin-mute)]">加载中…</div>
                ...
                <p className="px-3 py-2 text-xs text-[var(--twin-mute)]">
          ...
          {/* Pagination — audit tab only */}
          {tab === "audit" && canAudit && Math.max(auditTotal, restoredTotal) > auditSize ? (
            <div className="shrink-0 flex items-center justify-between gap-3 px-3 py-2 border-t border-[var(--app-color-border-default)] text-sm">
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  className="rounded-twin-sm border border-[var(--twin-hairline-strong)] px-2 py-1 text-xs disabled:opacity-40"
                ...
                <button
                  type="button"
                  className="rounded-twin-sm border border-[var(--twin-hairline-strong)] px-2 py-1 text-xs disabled:opacity-40"
                  disabled={auditPage >= maxAuditPages}
        ...
        </div>{/* table shadow container end */}
      </div>{/* max-h container end */}
    </AdminPageShell>
  );
}
