...
function RoomStatusCard({ config, activeStatus, activeTab }: { config: any, activeStatus?: RoomInventory, activeTab: string }) {
    ...
    return (
        <div className={`bg-[var(--app-color-surface-container)] rounded-2xl shadow-[var(--app-elevation-card)] border ${isExpanded ? `border-${themeColor}-300 ring-2 ring-${themeColor}-100` : 'border-[var(--app-color-border-default)]'} flex flex-col transition-all duration-300 group relative hover:z-50`}>
            {/* 卡片头部 */}
            ...
            <div className={`px-5 py-4 border-b rounded-t-2xl ${activeTab === '浦东' ? 'bg-blue-50/50 border-blue-100' : 'bg-emerald-50/50 border-emerald-100'} flex justify-between items-center relative`}>
                <div className="flex items-center gap-3 z-10 overflow-hidden">
                    <div className={`w-10 h-10 rounded-xl bg-[var(--app-color-surface-container)] flex items-center justify-center border ${activeTab === '浦东' ? 'border-blue-200' : 'border-emerald-200'} shadow-[var(--app-elevation-card)] shrink-0`}>
                        <Users className={`w-5 h-5 text-${themeColor}-600`} />
                    </div>
                    <div className="flex flex-col overflow-hidden">
                        <h3 className="text-xl font-black text-[var(--app-color-text-primary)] tracking-wide truncate">{config.roomName}</h3>
                        {config.mappingAliases && (
                            <span className="text-[9px] text-[var(--app-color-text-tertiary)] font-bold truncate max-w-[120px]" title={config.mappingAliases}>
                ...
                <div className="relative group/menu z-20 shrink-0">
                    ...
                    <div className="absolute right-0 top-full mt-1 w-[140px] bg-[var(--app-color-surface-container)] rounded-lg shadow-xl border border-[var(--app-color-border-default)] opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all duration-200 origin-top-right scale-95 group-hover/menu:scale-100 flex flex-col p-1.5 gap-1.5">

                        {/* ⚙️ 限载设置区 */}
                        <div className="flex items-center justify-between bg-[var(--app-color-surface-page)] p-1 rounded-md border border-[var(--app-color-border-default)]">
                            <span className="text-[10px] font-bold text-[var(--app-color-text-tertiary)] px-1 whitespace-nowrap">限载</span>
                            <input
                                type="number"
                                value={localCapacity}
                        ...
                        <div className="flex flex-col gap-1 bg-violet-50/80 p-1.5 rounded-md border border-violet-100">
                            ...
                            <div className="flex items-center gap-1">
                                <input
                                    ...
                                    className="flex-1 min-w-0 h-6 px-1 text-[10px] font-mono border border-violet-200 rounded bg-[var(--app-color-surface-container)]"
                                />
                                <button
                                    type="button"
                        ...
                        <button
                            ...
                            className="flex items-center justify-center gap-1.5 p-1.5 text-[11px] text-[var(--app-color-feedback-danger)] hover:bg-rose-50 rounded-md transition-colors w-full border border-transparent hover:border-rose-100"
                            title="抹除此房间"
                        >
            ...
            <div className="px-2 py-2 grid grid-cols-4 gap-2">
                <div className="flex flex-col items-center justify-center p-2 bg-[var(--app-color-surface-page)] rounded-lg">
                    <span className="text-[10px] font-bold text-[var(--app-color-text-tertiary)] mb-1">剩余卡</span>
                    <span className={`text-2xl font-black ${isFull ? 'text-[var(--app-color-feedback-danger)] animate-pulse' : `text-${themeColor}-600`}`}>{remainingCards}</span>
                </div>
                <div className="flex flex-col items-center justify-center p-2 bg-[var(--app-color-surface-page)] rounded-lg">
                    <span className="text-[10px] font-bold text-[var(--app-color-text-tertiary)] mb-1">自有卡</span>
                    <span className="text-2xl font-black text-[var(--app-color-text-secondary)]">{safeStatus.campusUserCount}</span>
                </div>
                <div className="flex flex-col items-center justify-center p-2 bg-[var(--app-color-surface-page)] rounded-lg">
                    <span className="text-[10px] font-bold text-[var(--app-color-text-tertiary)] mb-1">领卡</span>
                    <span className="text-2xl font-black text-[var(--app-color-text-secondary)]">{safeStatus.borrowedCardCount}</span>
                </div>
            </div>
            ...
            <button
                ...
                className="w-full py-2 bg-[var(--app-color-surface-page)] border-t border-[var(--app-color-border-default)] text-xs font-bold text-[var(--app-color-text-tertiary)] hover:bg-[var(--app-color-surface-hover)] hover:text-[var(--app-color-text-primary)] flex items-center justify-center gap-1 transition-colors"
            ...
            <AnimatePresence>
                {isExpanded && totalOccupants > 0 && (
                    <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden bg-[var(--app-color-surface-container)] border-t border-[var(--app-color-border-default)]">
                        <div className="p-4 flex flex-col gap-2 max-h-[250px] overflow-y-auto custom-scrollbar">
                            {safeStatus.occupants.map((occ, idx) => {
                                ...
                                return (
                                    <div key={idx} className="flex items-center justify-between bg-[var(--app-color-surface-page)] border border-[var(--app-color-border-default)] rounded-lg p-2.5 hover:border-[var(--app-color-border-strong)] transition-colors">
                                        <div className="flex items-center gap-3 overflow-hidden">
                                            <div className="w-8 h-8 rounded-full bg-[var(--app-color-surface-hover)] border border-[var(--app-color-border-strong)] flex items-center justify-center font-black text-[var(--app-color-text-secondary)] shrink-0 text-sm">
...
export default function DebugCardStatusPage() {
    ...
    if (isRoomsLoading)
        return (
            <div
                ...
                className="flex h-screen w-full animate-pulse items-center justify-center bg-[var(--app-color-surface-page)] font-bold text-[var(--app-color-text-tertiary)]"
            >
                正在唤醒底层 SQLite 数据总线...
            </div>
        );
    ...
    return (
        ...
        <div
            data-twin-debug-cards
            className="custom-scrollbar box-border h-full min-h-screen overflow-y-auto bg-[var(--app-color-surface-page)] p-8 pb-20 font-sans text-[var(--app-color-text-primary)]"
        >
            ...
            <AdminToolbar className="mb-6 flex shrink-0 flex-nowrap items-center gap-3 overflow-x-auto pb-1">
                <AdminToolbarPrimary className="min-w-0 max-w-[min(48vw,24rem)] shrink">
                    <h1 className="flex items-center gap-2 truncate text-2xl font-black text-[var(--app-color-text-primary)]">
                        <Activity className="h-7 w-7 shrink-0 text-[var(--app-color-accent)]" /> 房间监控
                    </h1>
                </AdminToolbarPrimary>
                <AdminToolbarActions className="ml-auto flex min-w-0 shrink-0 flex-nowrap items-center gap-2 sm:gap-3">
                    ...
                    <button
                        ...
                        className={`flex h-[var(--admin-control-height,2.25rem)] shrink-0 items-center gap-1.5 rounded-lg px-3 text-xs font-bold shadow-[var(--app-elevation-card)] transition-all active:scale-95 ${showAddPanel ? 'bg-[var(--app-color-feedback-danger-soft)] text-[var(--app-color-feedback-danger)] hover:bg-rose-200' : 'bg-indigo-100 text-indigo-700 hover:bg-indigo-200'}`}
                    >
                        {showAddPanel ? <X className="h-4 w-4" /> : <Edit3 className="h-4 w-4" />}
                        {showAddPanel ? '关闭配置' : '空间坐标'}
                    </button>
                    <div className="flex shrink-0 items-center rounded-xl border border-[var(--app-color-border-default)] bg-[var(--app-color-surface-container)] p-1 shadow-[var(--app-elevation-card)]">
                        {(['浦东', '浦西'] as const).map(tab => (
                            <button
                                ...
                                className={`rounded-lg px-4 py-2 text-xs font-black transition-all sm:px-6 sm:text-sm ${
                                    activeTab === tab
                                        ? (tab === '浦东' ? 'bg-[var(--app-color-accent)] text-white shadow-md' : 'bg-emerald-600 text-white shadow-md')
            ...
            <AnimatePresence>
                {showAddPanel && (
                    ...
                    >
                        <div className="flex flex-wrap items-center gap-3 bg-[var(--app-color-surface-container)] p-4 rounded-xl border border-[var(--app-color-border-default)] shadow-[var(--app-elevation-card)] relative">
                            <div className={`absolute top-0 left-0 w-1.5 h-full ${activeTab === '浦东' ? 'bg-blue-500' : 'bg-emerald-500'}`}></div>
                            <span className="text-xs font-black text-[var(--app-color-text-tertiary)] uppercase tracking-wider pl-2 mr-2">极速新增 {activeTab} 坐标</span>
                            ...
                            <input
                                ...
                                className="px-3 py-2 rounded-lg border border-[var(--app-color-border-strong)] text-sm font-bold outline-none focus:border-indigo-500 bg-[var(--app-color-surface-page)] focus:bg-[var(--app-color-surface-container)] w-40"
                            />
                            <input
                                type="number"
                                placeholder="限载"
                                ...
                                className="px-3 py-2 rounded-lg border border-[var(--app-color-border-strong)] text-sm font-bold outline-none focus:border-indigo-500 bg-[var(--app-color-surface-page)] focus:bg-[var(--app-color-surface-container)] w-20 text-center"
                            />
                            <div className="relative flex-1 min-w-[200px] max-w-md">
                                <LinkIcon className="absolute left-3 top-2.5 w-4 h-4 text-[var(--app-color-text-tertiary)]" />
                                <input
                                    type="text"
                                    placeholder="流水匹配别名 (逗号分隔)"
                                    value={newRoom.mappingAliases}
                                    onChange={e => setNewRoom({...newRoom, mappingAliases: e.target.value})}
                                    className="pl-9 pr-3 py-2 w-full rounded-lg border border-[var(--app-color-border-strong)] text-sm outline-none focus:border-indigo-500 bg-[var(--app-color-surface-page)] focus:bg-[var(--app-color-surface-container)]"
                                />
                            </div>
                            <input
                                type="text"
                                ...
                                className="px-3 py-2 rounded-lg border border-violet-200 text-sm font-mono outline-none focus:border-violet-500 bg-violet-50/50 focus:bg-[var(--app-color-surface-container)] min-w-[12rem] flex-1 max-w-md"
                                title="与 aro_access_log.room_id 一致；多个后室共限载时写多个 id，英文逗号、中文逗号或分号分隔"
                            />
                            <button
                                onClick={() => addMutation.mutate()}
                                disabled={!newRoom.roomName || addMutation.isPending}
                                className={`${activeTab === '浦东' ? 'bg-[var(--app-color-accent)] hover:bg-blue-700' : 'bg-emerald-600 hover:bg-emerald-700'} text-white px-5 py-2 rounded-lg text-sm font-black flex items-center gap-2 disabled:opacity-50 transition-colors shadow-[var(--app-elevation-card)]`}
                            >
            ...
            <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-4">
                {displayedRooms.length === 0 ? (
                    <div className="col-span-full flex flex-col items-center justify-center py-20 text-[var(--app-color-text-tertiary)] opacity-60">
