...
import { fetchMyMaterialRequests, fetchMaterialRequestDetail, fetchAllMaterialRequests, exportMaterialAuditTrail, type MaterialRequest } from "@/api/domains/material.api";
...
export default function MaterialAuditExportPage() {
  ...
  const handleExportRange = async () => {
    ...
    catch { toast.error("导出失败"); } finally { setRangeExporting(false); }
  };
  ...
  return (
    <div className="space-y-4 p-6">
      <div className="rounded-twin-xl border border-[var(--twin-hairline)] bg-[var(--twin-canvas)] p-4 shadow-twin-level-1">
        ...
        <p className="mt-1 text-xs text-[var(--twin-mute)]">个人审计：查看/导出个人申领明细。课题组审计：按课题组查看/导出申领明细。教职工可切换查询任意有记录的人员和课题组。</p>
        <div className="mt-3 flex gap-2">{tabBtn("personal", "个人审计")}{tabBtn("group", "课题组审计")}</div>
      </div>
      ...
      <section className="rounded-twin-xl border border-[var(--twin-hairline)] bg-[var(--twin-canvas)] p-4 shadow-twin-level-1">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          ...
          {mode === "single" ? (
            <>
              ...
              <button onClick={handleExportSingle} disabled={exporting || !selectedId} className="rounded-full bg-emerald-600 px-4 py-2 text-xs font-medium text-white disabled:opacity-50">{exporting ? "导出中…" : "导出 Excel"}</button>
            </>
          ) : (
            <>
              <span className="text-xs text-[var(--twin-mute)]">共 {displayRows.length} 行</span>
              <button onClick={handleExportRange} disabled={rangeExporting} className="rounded-full bg-emerald-600 px-4 py-2 text-xs font-medium text-white disabled:opacity-50">{rangeExporting ? "导出中…" : "导出区间 Excel"}</button>
            </>
          )}
        </div>
        ...
        {mode === "multi" && displayRows.length > 0 && (
          <div className="mt-3 overflow-x-auto rounded-twin-lg border border-[var(--twin-hairline)]">
            <table className="min-w-full text-xs"><thead className="bg-[var(--twin-canvas-soft)]"><tr>
              <th className="px-2 py-2 text-left">申领单号</th><th className="px-2 py-2 text-left">物品</th><th className="px-2 py-2">数量</th><th className="px-2 py-2">状态</th><th className="px-2 py-2 text-left">申领人</th><th className="px-2 py-2 text-left">课题组</th><th className="px-2 py-2 text-left">时间</th>
            </tr></thead>
              <tbody>{displayRows.map((r: any, i: number) => <tr key={i} className="hover:bg-[var(--twin-canvas-soft)]"><td className="px-2 py-2 font-mono text-[10px]">{r.requestId}</td><td className="px-2 py-2">{r.snapshotName}</td><td className="px-2 py-2 text-center">{r.qty}</td><td className="px-2 py-2 text-center">{statusZh(r.status)}</td><td className="px-2 py-2">{r.applicantName||"-"}</td><td className="px-2 py-2">{r.applicantGroup||"-"}</td><td className="px-2 py-2">{toTime(r.createdAt)}</td></tr>)}</tbody></table>
          </div>
        )}
        {mode === "multi" && displayRows.length === 0 && <p className="text-xs text-[var(--twin-mute)] py-4 text-center">该区间内暂无数据</p>}
      </section>
    </div>
  );
}
