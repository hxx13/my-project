...
import { fetchAllMaterialDemands, resolveMaterialDemand, type MaterialDemand } from "@/api/domains/material.api";
            ...
            <div key={d.id} className="rounded-twin-lg border border-[var(--twin-hairline)] bg-[var(--twin-canvas)] p-4 shadow-twin-level-1 flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs text-[var(--twin-mute)] font-mono">{d.userId}</span>
              ...
              {d.status === 0 && (
                <button onClick={async () => { await resolveMaterialDemand(d.id); qc.invalidateQueries({ queryKey: ["material", "demands"] }); toast.success("已标记为处理"); }}
              ...
              <div key={req.id} className="rounded-twin-lg border border-[var(--twin-hairline)] bg-[var(--twin-canvas)] p-4 shadow-twin-level-1 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {canDelete && (
                      <button onClick={() => { if (!window.confirm("删除此申领记录？")) return; /* soft-delete */ }}
                ...
                {(req.status === "PENDING" || req.status === "FIRST_OK") && (
                  <div className="flex gap-2 pt-1 border-t border-[var(--twin-hairline)]">
                    <button onClick={() => approve.mutate(req.id)} disabled={approve.isPending}
                      className="rounded-twin-sm bg-green-600 px-4 py-1.5 text-sm font-medium text-white hover:bg-green-700 disabled:opacity-50">
                      {req.status === "PENDING" && req.workflowType === "DUAL_REVIEW" ? "初审通过" : "通过"}
                    </button>
                    <button onClick={() => reject.mutate(req.id)} disabled={reject.isPending}
                      className="rounded-twin-sm bg-red-500 px-4 py-1.5 text-sm font-medium text-white hover:bg-red-600 disabled:opacity-50">拒绝</button>
                  </div>
                )}
                {req.status === "APPROVED" && (
                  <div className="flex gap-2 pt-1 border-t border-[var(--twin-hairline)]">
                    <button onClick={() => { const lines = req.lines?.map(l => ({ lineId: l.id, grant: true, fulfillQty: l.qty })) ?? []; fulfill.mutate({ id: req.id, lines }); }} disabled={fulfill.isPending}
                      className="rounded-twin-sm bg-blue-600 px-4 py-1.5 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50">确认出库</button>
                  </div>
                )}
              </div>
            ))}
