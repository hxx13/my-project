...
import {
  ...
  useCompleteRepair,
  useDeleteRepair,
  usePurgeRepairRecycle,
  usePurgeAllRepairRecycle,
  useRestoreRepairRecycle,
} from "@/api/hooks/useRepair";
...
export default function RepairProcessPage() {
  ...
  const {
    data: listData,
    isLoading,
  } = useRepairList({ page: 1, size: 100, includePrivate: true });

  const { data: recycleData, isLoading: recycleLoading } = useRepairRecycle({ page: 1, size: 100 });

  const rows: RepairOrderRecord[] = listData?.data ?? [];
  const recycleRows: RepairOrderRecord[] = recycleData?.data ?? [];
  ...
  const purgeSelectedMutation = usePurgeRepairRecycle();
  const purgeAllMutation = usePurgeAllRepairRecycle();
  const restoreMutation = useRestoreRepairRecycle();
  ...
  return (
    <div className="p-6">
      <section className="rounded-twin-lg border border-[var(--twin-hairline)] bg-[var(--twin-canvas)] p-5 shadow-twin-level-2">
        <h2 className="text-lg font-semibold text-[var(--twin-ink)] mb-4">报修处理台</h2>
        {isLoading ? (
          <DataSkeleton variant="table" rows={4} />
        ) : rows.length === 0 ? (
          <EmptyState title="暂无工单" description="当前没有待处理的报修工单" />
        ) : (
