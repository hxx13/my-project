...
import { useRepairList, useCreateRepair, useWithdrawRepair } from "@/api/hooks/useRepair";
...
export default function RepairRequestPage() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useRepairList({ page, size, status, dateFrom, dateTo });
  ...
  const withdrawMutation = useWithdrawRepair();
  ...
  return (
    <div className="p-6 space-y-6">
      <section className="rounded-twin-lg border border-[var(--twin-hairline)] bg-[var(--twin-canvas)] p-5 shadow-twin-level-2">
        <h2 className="text-lg font-semibold text-[var(--twin-ink)] mb-4">新增报修单</h2>
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <Button type="submit" disabled={createMutation.isPending}>
            {createMutation.isPending ? "提交中..." : "提交报修"}
          </Button>
        </form>
      </section>
      ...
      <section className="rounded-twin-lg border border-[var(--twin-hairline)] bg-[var(--twin-canvas)] p-5 shadow-twin-level-2">
        ...
        {isLoading ? (
          ...
          <EmptyState title="暂无报修记录" description="提交第一条报修单开始使用" />
        ) : (
