...
public interface CageQuotaMapper {
    ...
    /** 某 AUP 在某房间的配额（rent_number），无行返回 0 */
    int selectRentNumber(@Param("roomId") Long roomId, @Param("registerNo") String registerNo);

    /** 某房间已切出的配额之和（deleted=0） */
    int sumRentNumber(@Param("roomId") Long roomId);

    /** 某房间上限（animal_cage_number），无行返回 0 */
    int selectRoomCapacity(@Param("roomId") Long roomId);
