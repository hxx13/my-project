package com.example.demo.modules.facerecognition.djl;
...
/**
 * Ultra-Light 人脸检测 Translator（源自 DJL examples，Apache-2.0）
 */
public class FaceDetectionTranslator implements Translator<Image, DetectedObjects> {
    ...
    public static FaceDetectionTranslator ultraLightDefaults() {
        return new FaceDetectionTranslator(
                0.75,
                0.45,
                new double[] {0.1, 0.2},
                5000,
    ...
    public DetectedObjects processOutput(TranslatorContext ctx, NDList list) {
        ...
        prob = NDArrays.stack(new NDList(
        ...
        NDArray landms = decodeLandm(list.get(2), boxRecover, scaleXY);
    ...
    private NDArray boxRecover(NDManager manager, int width, int height, int[][] scales, int[] steps) {
        ...
        double[][] boxes = new double[defaultBoxes.size()][];
