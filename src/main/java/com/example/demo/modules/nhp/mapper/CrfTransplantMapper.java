...
public interface CrfTransplantMapper {
    ...
    @Insert("INSERT INTO crf_transplant (tx_code, project_name, remark, team_id, donor_subject_id, recipient_subject_id, xm_id, tx_organ, procedure_type, tx_date, cold_ischemia_min, warm_ischemia_min, reperfusion_time, induction_regimen, maintenance_regimen, parent_tx_id, status, created_by, active) VALUES (#{txCode}, #{projectName}, #{remark}, #{teamId}, #{donorSubjectId}, #{recipientSubjectId}, #{xmId}, #{txOrgan}, #{procedureType}, #{txDate}, #{coldIschemiaMin}, #{warmIschemiaMin}, #{reperfusionTime}, #{inductionRegimen}, #{maintenanceRegimen}, #{parentTxId}, #{status}, #{createdBy}, #{active})")
    ...
    @Update("UPDATE crf_transplant SET tx_code = #{txCode}, project_name = #{projectName}, remark = #{remark}, " +
            "team_id = #{teamId}, current_tp = #{currentTp}, stage_lock = #{stageLock}, donor_subject_id = #{donorSubjectId}, " +
