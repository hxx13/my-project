...
public interface CrfTransplantMapper {
    ...
    @Insert("INSERT INTO crf_transplant (tx_code, donor_subject_id, recipient_subject_id, xm_id, tx_organ, procedure_type, tx_date, cold_ischemia_min, warm_ischemia_min, reperfusion_time, induction_regimen, maintenance_regimen, parent_tx_id, status, active) VALUES (#{txCode}, #{donorSubjectId}, #{recipientSubjectId}, #{xmId}, #{txOrgan}, #{procedureType}, #{txDate}, #{coldIschemiaMin}, #{warmIschemiaMin}, #{reperfusionTime}, #{inductionRegimen}, #{maintenanceRegimen}, #{parentTxId}, #{status}, #{active})")
