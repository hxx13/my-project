...
public class PrintStationMapper {
    ...
    public void update(PrintStation s) {
        jdbc.update("UPDATE print_station SET name=?, user_id=?, page_size=?, supported_types=?, printer_ip=?, mode=?, enabled=? WHERE id=?",
                s.getName(), s.getUserId(), s.getPageSize(), s.getSupportedTypes(),
                s.getPrinterIp(), s.getMode(), s.isEnabled() ? 1 : 0, s.getId());
    }
