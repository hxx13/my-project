...
public class JobSchedulerService {
    ...
    private JobRunOutcome runWithStatus(String jobKey, String updatedBy, boolean skipIfRunning) {
        ...
        if (reaperJob) {
            com.example.demo.modules.twin.common.support.FreezeReaperAuditContext.begin(triggerType, updatedBy, jobKey);
        }
        ...
        try {
            ...
            if (reaperJob) {
                com.example.demo.modules.twin.common.support.FreezeReaperAuditContext.end();
            }
        }
    }
    ...
    private void initTelemetryArchivePurgeSchedule() {
        try {
            ...
            log.debug("初始化归档清理定时任务失败: {}", ignored.getMessage());
        }
    }
    ...
    private void migrateLegacyStatsPullScheduleJob() {
        try {
            ...
            // 迁移失败不阻断启动
            log.debug("迁移旧版统计拉取配置失败: {}", ignored.getMessage());
        }
    }
    ...
    private void disableDeprecatedScheduleJobs() {
        for (String key : JobExecutionRegistry.deprecatedJobKeys()) {
            try {
                ...
                log.debug("禁用废弃定时任务失败: {}", ignored.getMessage());
            }
        }
    }
    ...
    private void ensureColumnExists(String tableName, String columnName, String alterSql) {
        try {
            ...
            log.debug("检查列存在性失败: {}", ignored.getMessage());
        }
    }
