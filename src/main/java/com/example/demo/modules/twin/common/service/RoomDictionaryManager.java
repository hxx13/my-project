...
public class RoomDictionaryManager {
    ...
    private static final Logger log = LoggerFactory.getLogger(RoomDictionaryManager.class);
    ...
    public void initDictionary() {
        log.info("[字典装载] 正在解析多层级空间映射表 room_mapping.csv ...");
        ...
        try {
            ...
            reader.close();
            log.info("[字典装载] 满血完成！成功映射了 {} 个绝对空间坐标！", dictionary.size());

        } catch (Exception e) {
            log.error("[字典装载] 严重失败！无法读取 room_mapping.csv！报错：{}", e.getMessage());
        }
    }
