...
public class DahuaAutoSignoutService {
    ...
    /** 同一用户 + 触发原因的签退去重：避免 401 重试或多实例并发导致 ARO 重复记录 */
    private static final ConcurrentHashMap<String, Long> RECENT_SIGNOUT_AT_MS = new ConcurrentHashMap<>();
    private static final long RECENT_SIGNOUT_COOLDOWN_MS = 30_000L;
    ...
    public boolean autoSignout(String userId, String triggerType, String triggerReason, String detail) {
        ...
        if (lastSignoutMs != null && (nowMs - lastSignoutMs) < RECENT_SIGNOUT_COOLDOWN_MS) {
            ...
            writeAutoLog(userId, triggerType, triggerReason, true,
                    augmentLinkageSignoutDetail(triggerReason, dupDetail, true, userId));
            return true;
        }
