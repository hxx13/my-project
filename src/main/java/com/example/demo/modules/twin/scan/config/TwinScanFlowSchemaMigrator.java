...
public class TwinScanFlowSchemaMigrator {
    ...
    private static final Logger log = LoggerFactory.getLogger(TwinScanFlowSchemaMigrator.class);
    ...
    private void ensureColumn(String columnName, String alterSql) {
        try {
            ...
            log.info("[SchemaMigrator] 已补齐字段: aro_access_log.{}", columnName);
        } catch (Exception e) {
            log.warn("[SchemaMigrator] 字段补齐失败 {}: {}", columnName, e.getMessage());
        }
    }
}
