...
class CageStatusIntervalServiceTest {
    ...
    void firstCloseWithNullBeforeProducesNoInterval() {
        ...
        assertEquals(List.of(), service.foldRows(rows, SINCE));
    }
    ...
    void firstCloseWithExplicitClosedBeforeProducesNoInterval() {
        assertEquals(List.of(), service.foldRows(
                List.of(row(1L, "needs_division", "0", "0", at(0), "o1")), SINCE));
        assertEquals(List.of(), service.foldRows(
                List.of(row(1L, "needs_division", "false", "0", at(0), "o1")), SINCE));
    }
    ...
    void ignoresNonDataAndNonCageRows() {
        ...
        assertEquals(List.of(), service.foldRows(List.of(dict, otherTarget), SINCE));
    }
    ...
    @Test
    void foldDelegatesToMapperAndFolds() {
        CageFormAuditLog r = row(1L, "needs_division", "0", "1", at(0), "o1");
        when(mapper.listStatusFieldRows(anyList(), eq(SINCE))).thenReturn(List.of(r));

        List<StatusInterval> out = service.fold(SINCE);
